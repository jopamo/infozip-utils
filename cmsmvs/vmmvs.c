...
FILE* vmmvs_open_infile(__G) __GDEF {
    FILE* fzip;
        ...
        if (G.pInfo->textmode) {
            ...
            while (lef_len > EB_HEADSIZE) {
                ...
                if (eb_dlen > (lef_len - EB_HEADSIZE)) {
                    ...
                    TTrace((stderr, "open_outfile: block length %u > rest lef_size %u\n", eb_dlen, lef_len - EB_HEADSIZE));
                    break;
                }

                if ((eb_id == EF_VMCMS || eb_id == EF_MVS) && (getVMMVSexfield(type, lef_buf, eb_dlen) > 0)) {
                    mode = type;
                    break;
                }
...
unsigned datalen;
{
    ...
    strcpy(type, "w");
    strcat(type, fdata->__openmode == __TEXT ? "" : fdata->__openmode == __BINARY ? "b" : fdata->__openmode == __RECORD ? "b,type=record" : "");
    strcat(type, ",recfm=");
    strcat(type, fdata->__recfmF ? "F" : fdata->__recfmV ? "V" : fdata->__recfmU ? "U" : "?");
    if (fdata->__recfmBlk)
        strcat(type, "B");
    if (fdata->__recfmS)
        strcat(type, "S");
    ...
    if (fdata->__recfmM)
        strcat(type, "M");
    sprintf(type + strlen(type), ",lrecl=%ld", fdata->__recfmV ? fdata->__maxreclen + 4 : fdata->__maxreclen);
#ifdef VM_CMS
...
char* do_wild(__G__ wld) __GDEF ZCONST char* wld; /* only used first time on a given dir */
{
    static int First = 0;
...
int stat(const char* path, struct stat* buf) {
    ...
    if ((fp = fopen(path, FOPR)) != NULL) {
        ...
        time(&ltime);
        buf->st_atime = buf->st_mtime = buf->st_ctime = ltime;
...
void version(__G) __GDEF {
    ...
    VRM.iVRM = __librel();
    sprintf(liblvlmsg, "Using runtime library level %s V%dR%dM%d", (VRM.xVRM.pd == 1 ? "LE" : "CE"), VRM.xVRM.vv, VRM.xVRM.rr, VRM.xVRM.mm);
    ...
    sprintf(complevel, " V%dR%dM%d", VRM.xVRM.vv, VRM.xVRM.rr, VRM.xVRM.mm);
#else
#ifdef __IBMC__

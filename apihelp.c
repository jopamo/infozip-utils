...
APIDocStruct APIDoc[] = {{"UZPVERSION", "UzpVersion", "UzpVer *UzpVersion(void);",
                         ...
                         {"UZPMAIN", "UzpMain", "int UzpMain(int argc, char *argv[]);",
                         ...
                         {"UZPALTMAIN", "UzpAltMain", "int UzpAltMain(int argc, char *argv[], UzpInit *init);",
                          "Provide a direct entry point to the command line interface,\n"
                         ...
                         {"UZPUNZIPTOMEMORY", "UzpUnzipToMemory", "int UzpUnzipToMemory(char *zip, char *file, UzpBuffer *retstr);",
...
char* fname;
{
    ...
    if (!doc->compare)
        ...
        Info(slide, 0, ((char*)slide, "  Function:\t%s\n\n  Syntax:\t%s\n\n  Purpose:\t%s", doc->function, doc->syntax, doc->purpose));

    return 1;
}

void APIhelp(__G__ argc, argv) __GDEF int argc;
char** argv;
{
    if (argc > 1) {
        ...
        if (function_help(__G__ SYSTEM_API_DETAILS, argv[1]))
            return;
#endif
        Info(slide, 0, ((char*)slide, "%s is not a documented command.\n\n", argv[1]));
    }

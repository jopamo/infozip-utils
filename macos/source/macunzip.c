...
TrapType trapType;
{
    ...
    if ((trapType == ToolTrap) && (machineType > envMachUnknown) && (machineType < envMacII)) {
        ...
        case modifierMenu:
            switch (theitem) {
                ...
                case selectItem:
                    ...
                    if (theitem == okItem) {
                        GetDialogItem(thedialog, editItem, &itemType, &itemHandle, &itemRect);
...
char* argv[];
{
    ...
    sprintf(UnzipVersion, "%d.%d%d%s of %s", UZ_MAJORVER, UZ_MINORVER, UZ_PATCHLEVEL, UZ_BETALEVEL, UZ_VERSION_DATE);
    sprintf(ZipinfoVersion, "%d.%d%d%s of %s", ZI_MAJORVER, ZI_MINORVER, UZ_PATCHLEVEL, UZ_BETALEVEL, UZ_VERSION_DATE);
    ...
    if (markChar)
        modifiers ^= insensitiveFlag;
    GetItemMark(caseHandle, lowercaseItem, &markChar);
    if (markChar)
        modifiers ^= lowercaseFlag;
        ...
        if (command != ' ') {
            ...
            if (myReply.sfGood && (CheckMountedVolumes(ArchivePath) == 1)) {
                ...
                *s++ = '-';
                ...
                if (modifierMask) {
                    if (modifierMask & (autoFlag | textFlag))
                        *s++ = 'a';
                    if (modifierMask & textFlag)
                        *s++ = 'a';
                    if (modifierMask & insensitiveFlag)
                        *s++ = 'C';
                    if (modifierMask & junkFlag)
                        *s++ = 'j';
                    if (modifierMask & lowercaseFlag)
                        *s++ = 'L';
                    if (modifierMask & neverFlag)
                        *s++ = 'n';
                    if (modifierMask & overwriteFlag)
                        *s++ = 'o';
                    if (modifierMask & quietFlag)
                        *s++ = 'q';
                    if (modifierMask & verboseFlag)
                        *s++ = 'v';

                    if (modifierMask & prtCommentFlag)
                        *s++ = 'z';
                    if (modifierMask & prtHeaderFlag)
                        *s++ = 'h';
                    if (modifierMask & prtTotalsFlag)
                        *s++ = 't';
                    if (modifierMask & filenameFlag)
                        *s++ = '2';
                    if (modifierMask & longFlag)
                        *s++ = 'l';
                    if (modifierMask & mediumFlag)
                        *s++ = 'm';
                    if (modifierMask & shortFlag)
                        *s++ = 's';
                }

...
OSErr FSpFindFolder(short vRefNum,        /* Volume reference number. */
...
char* FindNewExtractFolder(char* ExtractPath, Boolean uniqueFolder) {
    ...
    AssertStr(ExtractPath, "FindNewExtractFolder ExtractPath == NULL")
        ...
        for (tmpPtr = ExtractPath; *tmpPtr; tmpPtr++) if (*tmpPtr == ':') {
        folderCount++;
        namePtr = tmpPtr;
    }
...
void FindDesktopFolder(char* Path) {
    ...
    for (i = 0; i < actVolCount; i++) {
        XGetVInfo(volumes[i].vRefNum, volumes[i].name, &volumes[i].vRefNum, &freeBytes, &totalBytes);
    ...
    FSpFindFolder(foundVRefNum, kDesktopFolderType, kDontCreateFolder, &spec);
...
char* TruncFilename(char* DirPath, const char* FilePath) {
    ...
    AssertStr(DirPath, "TruncFilename") Assert_it(Spec, "TruncFilename", "")
    ...
    if (dirPtr)
        *++dirPtr = '\0';
        ...
        printerr("TruncFilename: FilePath has no Folders", -1, -1, __LINE__, __FILE__, FilePath);
...
char* GetFilename(char* FileName, const char* FilePath) {
    ...
    Assert_it(FileName, "GetFilename", "") Assert_it(FilePath, "GetFilename", "")
...
char* GetFullPathFromID(char* CompletePath, short vRefNum, long dirID, ConstStr255Param name, OSErr* err) {
    FSSpec spec;

    *err = FSMakeFSSpecCompat(vRefNum, dirID, name, &spec);
    printerr("FSMakeFSSpecCompat:", (*err != -43) && (*err != 0), *err, __LINE__, __FILE__, "");
...
char* Real2RfDfFilen(char* RfDfFilen, const char* RealPath, short CurrentFork, short MacZipMode, Boolean DataForkOnly) {
    AssertStr(RealPath, "Real2RfDfFilen") AssertStr(RfDfFilen, "Real2RfDfFilen")
...
char* RfDfFilen2Real(char* RealFn, const char* RfDfFilen, short MacZipMode, Boolean DataForkOnly, short* CurrentFork) {
    short length;
    int result;

    AssertStr(RfDfFilen, "RfDfFilen2Real")

        if (DataForkOnly || (MacZipMode == UnKnown_EF) || (MacZipMode < JohnnyLee_EF)) {
    ...
    switch (MacZipMode) {
        ...
        case NewZipMode_EF: { /* determine Fork type */
            result = strncmp(RfDfFilen, ResourceMark, sizeof(ResourceMark) - 2);
            if (result != 0) {
                *CurrentFork = DataFork;
                sstrcpy(RealFn, RfDfFilen);
...
char* GetFullPathFromSpec(char* FullPath, FSSpec* Spec, OSErr* err) {
    ...
    printerr("FSpGetFullPath:", (*err != -43) && (*err != 0), *err, __LINE__, __FILE__, "");
    ...
    printerr("Warning path length exceeds limit: ", len >= NAME_MAX, len, __LINE__, __FILE__, " chars ");
...
char* GetCompletePath(char* CompletePath, const char* name, FSSpec* Spec, OSErr* err) {
    Boolean hasDirName = false;
    char currentdir[NAME_MAX];
    char* tmpPtr;
    unsigned short pathlen;

    AssertStr(name, "GetCompletePath") Assert_it(Spec, "GetCompletePath", "") Assert_it((CompletePath != name), "GetCompletePath", "")

        for (tmpPtr = name; *tmpPtr; tmpPtr++) if (*tmpPtr == ':') hasDirName = true;

    if (name[0] != ':') /* case c: path including volume name or only filename */
    {
        if (hasDirName) { /* okey, starts with volume name, so it must be a complete path */
            sstrcpy(CompletePath, name);
        }
        else { /* only filename: add cwd and return */

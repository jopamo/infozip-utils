...
static Boolean HeaderIsMacBinary(char* header, Word* version, long maxDataLen);
...
static Boolean HeaderIsMacBinary(char* header, Word* version, long maxDataLen) {
    ...
    else if (BYTE_AT_OFFSET(header, kVersionCheckZero) == 0 && BYTE_AT_OFFSET(header, kOldVersionOffset) == 0) {
        if (WORD_AT_OFFSET(header, kCRCOffset) == crc32(0, (unsigned char*)&header, 124)) {
            isIt = true;
            mbVersion = 129;
        }
        else {
    ...
    printf("\n                                                 1. isIt: bool: %d", isIt);
    printf("\n BYTE_AT_OFFSET(header, kFileNameLengthOffset) >= 1       bool: %d", BYTE_AT_OFFSET(header, kFileNameLengthOffset) >= 1);
    printf("\n BYTE_AT_OFFSET(header, kFileNameLengthOffset) <= 31      bool: %d", BYTE_AT_OFFSET(header, kFileNameLengthOffset) <= 31);
    printf("\n dataForkLength >= 0                                      bool: %d", dataForkLength >= 0);
    printf("\n resourceForkLength >= 0                                  bool: %d", resourceForkLength >= 0);
    printf("\n resourceForkLength <= 0x%8x                         bool: %d", kResourceForkMaxLen, resourceForkLength <= kResourceForkMaxLen);
    printf("\n dataForkLength     <= %8u                           bool: %d", maxDataLen, dataForkLength <= maxDataLen);
    printf("\n df_rf_length        > 0                                  bool: %d", df_rf_length > 0);
    printf("\n df_rf_length       <= %8u                           bool: %d", maxDataLen, df_rf_length <= maxDataLen);
#endif

    if (isIt && BYTE_AT_OFFSET(header, kFileNameLengthOffset) >= 1 && BYTE_AT_OFFSET(header, kFileNameLengthOffset) <= 31 && dataForkLength >= 0 && resourceForkLength >= 0 &&
        resourceForkLength <= kResourceForkMaxLen && df_rf_length > 0 && df_rf_length <= maxDataLen) {
        isIt = true;
    }
    else
...
static Handle EncodeMacbinary(FSSpec* file) {
    ...
    WORD_AT_OFFSET(header, kCRCOffset) = crc32(0, (unsigned char*)&header, 124);

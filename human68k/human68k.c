    ...
    if (renamed) {
        ...
        while (*++cp)
            if (*cp == '\\') /* convert backslashes to forward */
                *cp = '/';
        cp = G.filename;
        if ((G.filename[0] == '/') || (isalpha(G.filename[0]) && G.filename[1] == ':')) {
    ...
    *pp = '\0'; /* done with pathcomp:  terminate it */
    ...
    if (G.pInfo->vollabel) { /* set the volume label now */
        ...
        if ((fd = _dos_newfile(G.filename, G.pInfo->file_attr)) < 0) {
            Info(slide, 1, ((char*)slide, "mapname:  error setting volume label\n"));
            return (error & ~MPN_MASK) | MPN_ERR_SKIP;
        }
        _dos_close(fd);
...
#define FUNCTION (flag & FN_MASK)
    ...
    if (FUNCTION == ROOT) {
        ...
        if ((rootlen = strlen(pathcomp)) > 0) {
            ...
            else if (rootlen > 0 && (SSTAT(tmproot, &G.statbuf) || !S_ISDIR(G.statbuf.st_mode))) /* path does not exist */
            {
...
void close_outfile(__G) __GDEF {
    ...
    if (uO.D_flag <= 1) {
        ...
        if (G.extra_field &&
            ...
            if (z_utime.mtime & 1)
                z_utime.mtime = (z_utime.mtime + 1 > z_utime.mtime) ? z_utime.mtime + 1 : z_utime.mtime - 1;
...
time_t modtime;
{
    ...
    if (((fd = open((char*)fname, 0)) == -1) || (_dos_filedate(fd, dos_dt.z_dostime))) {
        if (fd != -1)
            close(fd);
        return -1;
    }

 ...
 * of characters in the pattern. */
{
    ...
    if (c == WILDCHAR)
        ...
        return (*s && (!sepc || *s != (uch)sepc)) ? recmatch(p, s + CLEN(s), ic, sepc) : 0;
#else
    ...
    if (c == '*') {
        ...
        if (sepc) {
            ...
            if (*p != '*') {
                ...
                return (*p == (uch)sepc || (*p == '\\' && p[1] == (uch)sepc)) ? recmatch(p, s, ic, sepc) : 2;
            }
...
#ifdef QDOS
    return QMatch(Case((uch)c), Case(*s)) ? recmatch(p, s + CLEN(s), ic __WDL) : 0;
#else
...
int main(int argc, char** argv) {
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            if (!str[0])
                break;
            printf("Case sensitive: %s  insensitive: %s\n", match(str, pat, 0) ? "YES" : "NO", match(str, pat, 1) ? "YES" : "NO");
        }
    }
    EXIT(0);
}

...
static ZCONST char Far ZipInfHeader2[] = "Zip file size: %s bytes, number of entries: %s\n";
...
static ZCONST char Far ExtraBytesPreceding[] = "  There are an extra %s bytes preceding this file.\n\n";
...
static ZCONST char Far UT_FileModDate[] = "  file last modified on (UT extra field modtime): %s %s\n";
static ZCONST char Far LocalTime[] = "local";
#ifndef NO_GMTIME
static ZCONST char Far GMTime[] = "UTC";
#endif
#endif /* USE_EF_UT_TIME */
static ZCONST char Far CRC32Value[] = "  32-bit CRC value (hex):                         %.8lx\n";
...
static ZCONST char Far* izVMScomp[4] = {izVMSstored, izVMSrleenc, izVMSdeflat, izVMScunknw};
...
static ZCONST char Far* TandemFileformat[6] = {TandemUnstr, TandemRel, TandemEntry, TandemKey, TandemEdit, TandemObj};
...
static ZCONST char Far lExtraFieldType[] =
  ...
  %u data bytes (%s).\n";
static ZCONST char Far efIZuid[] = "GMT modification/access times and Unix UID/GID";
...
void zi_end_central(__G) __GDEF {
    /*---------------------------------------------------------------------------
    ...
    if (uO.lflag > 9) {
        ...
        Info(slide, 0, ((char*)slide, LoadFarString(ZipFSizeVerbose), FmZofft(G.ziplen, "11", NULL), FmZofft(G.ziplen, FZOFFT_HEX_DOT_WID, "X")));
        Info(slide, 0,
             ((char*)slide, LoadFarString(ActOffsetCentDir), FmZofft(G.real_ecrec_offset, "11", "u"), FmZofft(G.real_ecrec_offset, FZOFFT_HEX_DOT_WID, "X"), FmZofft(G.expect_ecrec_offset, "11", "u"),
              FmZofft(G.expect_ecrec_offset, FZOFFT_HEX_DOT_WID, "X")));

        if (G.ecrec.number_this_disk == 0) {
            Info(slide, 0,
                 ((char*)slide, LoadFarString(SinglePartArchive1), FmZofft(G.ecrec.total_entries_central_dir, NULL, "u"), (G.ecrec.total_entries_central_dir == 1) ? "entry" : "entries",
                  FmZofft(G.ecrec.size_central_directory, NULL, "u"), FmZofft(G.ecrec.size_central_directory, FZOFFT_HEX_DOT_WID, "X")));
            Info(slide, 0,
                 ((char*)slide, LoadFarString(SinglePartArchive2), FmZofft(G.ecrec.offset_start_central_directory, NULL, "u"),
                  FmZofft(G.ecrec.offset_start_central_directory, FZOFFT_HEX_DOT_WID, "X")));
        }
        else {
            Info(slide, 0, ((char*)slide, LoadFarString(MultiPartArchive1), (ulg)(G.ecrec.number_this_disk + 1), (ulg)(G.ecrec.num_disk_start_cdir + 1)));
            Info(slide, 0,
                 ((char*)slide, LoadFarString(MultiPartArchive2), FmZofft(G.ecrec.offset_start_central_directory, NULL, "u"), FmZofft(G.ecrec.offset_start_central_directory, FZOFFT_HEX_DOT_WID, "X"),
                  FmZofft(G.ecrec.size_central_directory, NULL, "u"), FmZofft(G.ecrec.size_central_directory, FZOFFT_HEX_DOT_WID, "X")));
            Info(slide, 0,
                 ((char*)slide, LoadFarString(MultiPartArchive3), FmZofft(G.ecrec.num_entries_centrl_dir_ths_disk, NULL, "u"), (G.ecrec.num_entries_centrl_dir_ths_disk == 1) ? "is" : "are",
                  FmZofft(G.ecrec.total_entries_central_dir, NULL, "u"), (G.ecrec.total_entries_central_dir == 1) ? "entry" : "entries"));
        }
    }
    else if (uO.hflag) {
        /* print zip file size and number of contained entries: */
        Info(slide, 0, ((char*)slide, LoadFarString(ZipInfHeader2), FmZofft(G.ziplen, NULL, NULL), FmZofft(G.ecrec.total_entries_central_dir, NULL, "u")));
    }
...
int zipinfo(__G) /* return PK-type error code */
    ...
    if (G.filespecs > 0 && (fn_matched = (int*)malloc(G.filespecs * sizeof(int))) != NULL)
        for (j = 0; j < G.filespecs; ++j)
            fn_matched[j] = FALSE;

    if (G.xfilespecs > 0 && (xn_matched = (int*)malloc(G.xfilespecs * sizeof(int))) != NULL)
    ...
    for (j = 1L;; j++) {
        ...
        if (memcmp(G.sig, central_hdr_sig, 4)) { /* is it a CentDir entry? */
             ...
             */
            if (((j - 1) & (ulg)(G.ecrec.have_ecr64 ? MASK_ZUCN64 : MASK_ZUCN16)) == (ulg)G.ecrec.total_entries_central_dir) {
        ...
        if (G.process_all_files || do_this_file) {
            ...
            switch (uO.lflag) {
                ...
                case 10:
                    Info(slide, 0, ((char*)slide, LoadFarString(CentralDirEntry), j));
                    if ((error = zi_long(__G__ & endprev, error_in_archive)) != PK_COOL) {
                        error_in_archive = error; /* might be warning */
                    }
                    break;
...
#ifdef DLL
            if ((G.statreportcb != NULL) && (*G.statreportcb)(__G__ UZ_ST_FINISH_MEMBER, G.zipfn, G.filename, (zvoid*)&G.crec.ucsize)) {
    ...
    if ((error_in_archive <= PK_WARN) && uO.tflag) {
        ...
        Info(
            slide, 0,
            ((char*)slide, LoadFarString(ZipfileStats), members, (members == 1L) ? nullStr : PlurSufx, FmZofft(tot_ucsize, NULL, "u"), FmZofft(tot_csize, NULL, "u"), sgn, cfactor / 10, cfactor % 10));
    }
    ...
    if (fn_matched) {
        if (error_in_archive <= PK_WARN)
            for (j = 0; j < G.filespecs; ++j)
                if (!fn_matched[j])
                    Info(slide, 0x401, ((char*)slide, LoadFarString(FilenameNotMatched), G.pfnames[j]));
    ...
    if (xn_matched) {
        if (error_in_archive <= PK_WARN)
            for (j = 0; j < G.xfilespecs; ++j)
                if (!xn_matched[j])
                    Info(slide, 0x401, ((char*)slide, LoadFarString(ExclFilenameNotMatched), G.pxnames[j]));
    ...
    if (error_in_archive <= PK_WARN) {
        ...
        if ((memcmp(G.sig, (G.ecrec.have_ecr64 ? end_central64_sig : end_central_sig), 4) != 0) && (!G.ecrec.is_zip64_archive) &&
            (memcmp(G.sig, end_central_sig, 4) != 0)) { /* just to make sure again */
...
int error_in_archive; /* may signal premature return */
{
    ...
    char unkn[16];
    static ZCONST char Far* os[NUM_HOSTS] = {OS_FAT,  OS_Amiga, OS_VMS,   OS_Unix, OS_VMCMS, OS_AtariST, OS_HPFS,   OS_Macintosh, OS_ZSystem,   OS_CPM, OS_TOPS20,
                                             OS_NTFS, OS_QDOS,  OS_Acorn, OS_VFAT, OS_MVS,   OS_BeOS,    OS_Tandem, OS_Theos,     OS_MacDarwin, NULL,   NULL,
                                             NULL,    NULL,     NULL,     NULL,    NULL,     NULL,       NULL,      NULL,         OS_AtheOS};
    static ZCONST char Far* method[NUM_METHODS] = {MthdNone,     MthdShrunk,     MthdRedF1, MthdRedF2, MthdRedF3, MthdRedF4, MthdImplode, MthdToken, MthdDeflate,
                                                   MthdDeflat64, MthdDCLImplode, MthdBZip2, MthdLZMA,  MthdTerse, MthdLZ77,  MthdWavPack, MthdPPMd};
    static ZCONST char Far* dtypelng[4] = {DeflNorm, DeflMax, DeflFast, DeflSFast};
    ...
    if (G.crec.relative_offset_local_header != *pEndprev && *pEndprev > 0L) {
         ...
         */
        Info(slide, 0, ((char*)slide, LoadFarString(ExtraBytesPreceding), FmZofft((G.crec.relative_offset_local_header - (*pEndprev)), NULL, NULL)));
    }
    ...
    *pEndprev = G.crec.relative_offset_local_header + (4L + LREC_SIZE) + G.crec.filename_length + G.crec.extra_field_length + G.crec.csize;
    ...
    if (hostnum >= NUM_HOSTS) {
        sprintf(unkn, LoadFarString(UnknownNo), (int)G.crec.version_made_by[1]);
        varmsg_str = unkn;
    }
    else {
    ...
    if ((extnum >= NUM_HOSTS) || (os[extnum] == NULL)) {
        sprintf(unkn, LoadFarString(UnknownNo), (int)G.crec.version_needed_to_extract[1]);
        varmsg_str = unkn;
    }
    else {
    ...
    if (methid == IMPLODED) {
        Info(slide, 0, ((char*)slide, LoadFarString(SlideWindowSizeImplode), (G.crec.general_purpose_bit_flag & 2) ? '8' : '4'));
        Info(slide, 0, ((char*)slide, LoadFarString(ShannonFanoTrees), (G.crec.general_purpose_bit_flag & 4) ? '3' : '2'));
    }
    else if (methid == DEFLATED || methid == ENHDEFLATED) {
        ush dnum = (ush)((G.crec.general_purpose_bit_flag >> 1) & 3);

        Info(slide, 0, ((char*)slide, LoadFarString(CompressSubtype), LoadFarStringSmall(dtypelng[dnum])));
    }
...
#define d_t_buf attribs
    ...
    if (G.extra_field &&
        ...
        G.tz_is_valid &&
#endif
        (ef_scan_for_izux(G.extra_field, G.crec.extra_field_length, 1, G.crec.last_mod_dos_datetime, &z_utime, NULL) & EB_UT_FL_MTIME)) {
        ...
        Info(slide, 0, ((char*)slide, LoadFarString(UT_FileModDate), d_t_buf, LoadFarStringSmall(LocalTime)));
#ifndef NO_GMTIME
        d_t_buf[0] = (char)1; /* signal "show UTC (GMT) time" */
        zi_time(__G__ & G.crec.last_mod_dos_datetime, &(z_utime.mtime), d_t_buf);
        Info(slide, 0, ((char*)slide, LoadFarString(UT_FileModDate), d_t_buf, LoadFarStringSmall(GMTime)));
#endif /* !NO_GMTIME */
    }
#endif /* USE_EF_UT_TIME */
    ...
    Info(slide, 0,
         ((char*)slide, LoadFarString(ApparentFileType),
    ...
    if (hostnum == VMS_) {
        ...
        *p = ')'; /* overwrite last comma */
        Info(slide, 0, ((char*)slide, LoadFarString(VMSFileAttributes), xattr, attribs));
    }
        ...
        Info(slide, 0, ((char*)slide, LoadFarString(AmigaFileAttributes), xattr, attribs));
    }
    ...
    else if ((hostnum != FS_FAT_) && (hostnum != FS_HPFS_) && (hostnum != FS_NTFS_) && (hostnum != FS_VFAT_) && (hostnum != ACORN_) && (hostnum != VM_CMS_) &&
             (hostnum != MVS_)) { /* assume Unix-like */
        switch ((unsigned)(xattr & UNX_IFMT)) {
            case (unsigned)UNX_IFDIR:
                attribs[0] = 'd';
                break;
            case (unsigned)UNX_IFREG:
                attribs[0] = '-';
                break;
            case (unsigned)UNX_IFLNK:
                attribs[0] = 'l';
                break;
            case (unsigned)UNX_IFBLK:
                attribs[0] = 'b';
                break;
            case (unsigned)UNX_IFCHR:
                attribs[0] = 'c';
                break;
            case (unsigned)UNX_IFIFO:
                attribs[0] = 'p';
                break;
            case (unsigned)UNX_IFSOCK:
                attribs[0] = 's';
                break;
            default:
                attribs[0] = '?';
        ...
        Info(slide, 0, ((char*)slide, LoadFarString(UnixFileAttributes), xattr, attribs));
    }
    else {
        Info(slide, 0, ((char*)slide, LoadFarString(NonMSDOSFileAttributes), G.crec.external_file_attributes >> 8));

    } /* endif (hostnum: external attributes format) */

    if ((xattr = (unsigned)(G.crec.external_file_attributes & 0xFF)) == 0)
        Info(slide, 0, ((char*)slide, LoadFarString(MSDOSFileAttributes), xattr));
    else if (xattr == 1)
        Info(slide, 0, ((char*)slide, LoadFarString(MSDOSFileAttributesRO), xattr));
    else
        Info(slide, 0,
             ((char*)slide, LoadFarString(MSDOSFileAttributesAlpha), xattr, (xattr & 1) ? "rdo " : nullStr, (xattr & 2) ? "hid " : nullStr, (xattr & 4) ? "sys " : nullStr,
              (xattr & 8) ? "lab " : nullStr, (xattr & 16) ? "dir " : nullStr, (xattr & 32) ? "arc " : nullStr, (xattr & 64) ? "lnk " : nullStr, (xattr & 128) ? "exe" : nullStr));
    ...
    if (G.crec.extra_field_length > 0) {
        ...
        while (ef_len >= EB_HEADSIZE) {
            ...
            if (eb_datalen > (ush)ef_len) {
                Info(slide, 0x421, ((char*)slide, LoadFarString(ExtraFieldTrunc), eb_id, eb_datalen, ef_len));
                eb_datalen = ef_len;
            }
            ...
            switch (eb_id) {
                case EF_PKSZ64:
                    ...
                    if ((G.crec.relative_offset_local_header & (~(zusz_t)0xFFFFFFFFL)) != 0) {
            ...
            Info(slide, 0, ((char*)slide, LoadFarString(ExtraFieldType), eb_id, LoadFarStringSmall(ef_fieldname), eb_datalen));
            ...
            switch (eb_id) {
                ...
                case EF_ACL:
                    if (eb_datalen >= EB_OS2_HLEN) {
                        ...
                        Info(slide, 0, ((char*)slide, LoadFarString(ef_fieldname), makelong(ef_ptr)));
                ...
                case EF_NTSD:
                    if (eb_datalen >= EB_NTSD_C_LEN) {
                        Info(slide, 0, ((char*)slide, LoadFarString(NTSDData), makelong(ef_ptr)));
                        *pEndprev = 0L; /* no clue about csize of local */
                    }
                    else {
                        goto ef_default_display;
                ...
                case EF_IZVMS:
                    if (eb_datalen >= 8) {
                        char *p, q[8];
                        unsigned compr = makeword(ef_ptr + EB_IZVMS_FLGS) & EB_IZVMS_BCMASK;
                        ...
                        *q = '\0';
                        ...
                        Info(slide, 0, ((char*)slide, LoadFarString(izVMSdata), LoadFarStringSmall(izVMScomp[compr]), makeword(ef_ptr + EB_IZVMS_UCSIZ), p, q));
                    }
                    else {
                        goto ef_default_display;
                    }
                ...
                case EF_TIME:
                    if (eb_datalen > 0) {
                        ...
                        *types = '\0';
                        ...
                        if (num > 0)
                            Info(slide, 0, ((char*)slide, LoadFarString(UTdata), types, num == 1 ? nullStr : PlurSufx));
                    }
                    break;
                case EF_UNIPATH:
                case EF_UNICOMNT:
                    if (eb_datalen >= 5) {
                        ...
                        if (eb_datalen <= 29) {
                            Info(slide, 0, ((char*)slide, LoadFarString(U8PthCmnComplete), (unsigned)ef_ptr[0], name_crc));
                            n = eb_datalen;
                        }
                        else {
                            Info(slide, 0, ((char*)slide, LoadFarString(U8PthCmnF24), (unsigned)ef_ptr[0], name_crc));
                            n = 29;
                        }
                        for (i = 5; i < n; ++i)
                            Info(slide, 0, ((char*)slide, LoadFarString(efFormat), ef_ptr[i]));
                    }
                    else {
                        goto ef_default_display;
                    }
                ...
                case EF_MAC3:
                    if (eb_datalen >= EB_MAC3_HLEN) {
                        ...
                        Info(slide, 0, ((char*)slide, LoadFarString(Mac3data), eb_uc, eb_is_uc ? "un" : nullStr));
                        if (eb_is_uc) {
                            if (*pEndprev > 0L)
                        ...
                        Info(slide, 0, ((char*)slide, LoadFarString(MacOSMAC3flags), LoadFarStringSmall(mac3_flgs & EB_M3_FL_DATFRK ? MacOS_DF : MacOS_RF), (mac3_flgs & EB_M3_FL_TIME64 ? 64 : 32)));
                    ...
                    break;
                case EF_ZIPIT2:
                    if (eb_datalen >= 5 && makelong(ef_ptr) == 0x5449505A /* "ZPIT" */) {
                        if (eb_datalen >= 12) {
                ...
                case EF_ZIPIT:
                    if (eb_datalen >= 5 && makelong(ef_ptr) == 0x5449505A /* "ZPIT" */) {
                        ...
                        if ((unsigned)eb_datalen >= fnlen + (5 + 8)) {
                            ...
                            A_TO_N(ef_ptr + 5);
                            Info(slide, 0, ((char*)slide, LoadFarString(ZipItFname), (char*)ef_ptr + 5));
                    ...
                    break;
                case EF_JLMAC:
                    if (eb_datalen >= 40 && makelong(ef_ptr) == 0x45454C4A /* "JLEE" */) {
                        zi_showMacTypeCreator(__G__ & ef_ptr[4]);

                        Info(slide, 0, ((char*)slide, LoadFarString(MacOSJLEEflags), LoadFarStringSmall(ef_ptr[31] & 1 ? MacOS_DF : MacOS_RF)));
                    }
                    else {
                        goto ef_default_display;
                    }
                    break;
                case EF_SMARTZIP:
                    if ((eb_datalen == EB_SMARTZIP_HLEN) && makelong(ef_ptr) == 0x70695A64 /* "dZip" */) {
                        char filenameBuf[32];
                        zi_showMacTypeCreator(__G__ & ef_ptr[4]);
                        memcpy(filenameBuf, &ef_ptr[33], 31);
                        filenameBuf[ef_ptr[32]] = '\0';
                        A_TO_N(filenameBuf);
                        Info(slide, 0, ((char*)slide, LoadFarString(ZipItFname), filenameBuf));
                    }
                    else {
                        goto ef_default_display;
                    }
                ...
                case EF_MVS: {
                    ...
                    Info(slide, 0, ((char*)slide, LoadFarString(VmMvsExtraField), (getVMMVSexfield(type, ef_ptr - EB_HEADSIZE, (unsigned)eb_datalen) > 0) ? type : LoadFarStringSmall(VmMvsInvalid)));
                } break;
#endif /* CMS_MVS */
                case EF_ATHEOS:
                case EF_BEOS:
                    if (eb_datalen >= EB_BEOS_HLEN) {
                        ulg eb_uc = makelong(ef_ptr);
                        unsigned eb_is_uc = *(ef_ptr + EB_FLGS_OFFS) & EB_BE_FL_UNCMPR;

                        if (eb_id == EF_ATHEOS)
                            ef_fieldname = AtheOSdata;
                        else
                            ef_fieldname = BeOSdata;
                        Info(slide, 0, ((char*)slide, LoadFarString(ef_fieldname), eb_uc, eb_is_uc ? "un" : nullStr));
                        if (eb_is_uc) {
                            if (*pEndprev > 0L)
                                *pEndprev += makelong(ef_ptr);
                        }
                ...
                case EF_QDOS:
                    if (eb_datalen >= 4) {
                        Info(slide, 0, ((char*)slide, LoadFarString(QDOSdata), ef_ptr[0], ef_ptr[1], ef_ptr[2], ef_ptr[3]));
                    }
                    else {
                        goto ef_default_display;
                    }
                ...
                case EF_AOSVS:
                    if (eb_datalen >= 5) {
                        Info(slide, 0, ((char*)slide, LoadFarString(AOSVSdata), ((int)(uch)ef_ptr[4]) / 10, ((int)(uch)ef_ptr[4]) % 10));
                    }
                    else {
                        goto ef_default_display;
                    }
                ...
                case EF_TANDEM:
                    if (eb_datalen == 20) {
                        ...
                        Info(slide, 0, ((char*)slide, LoadFarString(Tandemdata), LoadFarStringSmall(TandemFileformat[type]), code));
                    }
                    else {
                        goto ef_default_display;
                    }
                ...
                case EF_MD5:
                    if (eb_datalen >= 19) {
                        ...
                        md5[32] = '\0';
                        Info(slide, 0, ((char*)slide, LoadFarString(MD5data), md5));
                        break;
                    } /* else: fall through !! */
                default:
                ef_default_display:
                    if (eb_datalen > 0) {
                        ...
                        if (eb_datalen <= 24) {
                            ...
                            Info(slide, 0, ((char*)slide, LoadFarString(First20)));
                            n = 20;
                        }
                        for (i = 0; i < n; ++i)
                            Info(slide, 0, ((char*)slide, LoadFarString(efFormat), ef_ptr[i]));
    ...
    if ((xattr = (unsigned)((G.crec.external_file_attributes & 0xC000) >> 12)) & 8) {
        if (hostnum == UNIX_ || hostnum == FS_HPFS_ || hostnum == FS_NTFS_) {
            Info(slide, 0, ((char*)slide, LoadFarString(lExtraFieldType), "is", EF_IZUNIX, LoadFarStringSmall(efIZUnix), (unsigned)(xattr & 12), (xattr & 4) ? efIZuid : efIZnouid));
            if (*pEndprev > 0L)
                *pEndprev += (ulg)(xattr & 12);
        }
        else if (hostnum == FS_FAT_ && !(xattr & 4))
            Info(slide, 0, ((char*)slide, LoadFarString(lExtraFieldType), "may be", EF_IZUNIX, LoadFarStringSmall(efIZUnix), 8, efIZnouid));
    }
...
static int zi_short(__G) /* return PK-type error code */
    ...
    static ZCONST char Far os[NUM_HOSTS + 1][4] = {"fat", "ami", "vms", "unx", "cms", "atr", "hpf", "mac", "zzz", "cpm", "t20", "ntf", "qds", "aco", "vft", "mvs",
                                                   "be ", "nsk", "ths", "osx", "???", "???", "???", "???", "???", "???", "???", "???", "???", "???", "ath", "???"};
#ifdef OLD_THEOS_EXTRA
    static ZCONST char Far os_TheosOld[] = "tho";
#endif
    static ZCONST char Far method[NUM_METHODS + 1][5] = {"stor", "shrk", "re:1", "re:2", "re:3", "re:4", "i#:#", "tokn", "def#",
                                                         "d64#", "dcli", "bzp2", "lzma", "ters", "lz77", "wavp", "ppmd", "u###"};
    ...
    switch (hostnum) {
        ...
        case ACORN_:
            if (hostnum != FS_FAT_ ||
                (unsigned)(xattr & 0700) != ((unsigned)0400 | ((unsigned)!(G.crec.external_file_attributes & 1) << 7) | ((unsigned)(G.crec.external_file_attributes & 0x10) << 2))) {
                ...
                if (IS_VOLID(xattr))
                    ...
                    ++p;
                    if (STRNICMP(p, "com", 3) == 0 || STRNICMP(p, "exe", 3) == 0 || STRNICMP(p, "btm", 3) == 0 || STRNICMP(p, "cmd", 3) == 0 || STRNICMP(p, "bat", 3) == 0)
                        attribs[3] = 'x';
                }
                break;
            } /* else: fall through! */
...
#ifdef OLD_THEOS_EXTRA
    Info(slide, 0, ((char*)slide, "%s %s %s ", attribs, LoadFarStringSmall(((hostnum == FS_VFAT_ && hostver == 20) ? os_TheosOld : os[hostnum])), FmZofft(G.crec.ucsize, "8", "u")));
#else
    Info(slide, 0, ((char*)slide, "%s %s %s ", attribs, LoadFarStringSmall(os[hostnum]), FmZofft(G.crec.ucsize, "8", "u")));
#endif
    Info(slide, 0,
         ((char*)slide, "%c",
          (G.crec.general_purpose_bit_flag & 1) ? ((G.crec.internal_file_attributes & 1) ? 'T' : 'B') : /* encrypted */
    ...
    k = (G.crec.extra_field_length ||
         ...
         ((G.crec.external_file_attributes & 0x8000) && (hostnum == UNIX_ || hostnum == FS_HPFS_ || hostnum == FS_NTFS_)));
    Info(slide, 0,
         ((char*)slide, "%c",
          k ? ((G.crec.general_purpose_bit_flag & 8) ? 'X' : 'x') :  /* extra field */
    ...
    if (uO.lflag == 4) {
        ...
        Info(slide, 0, ((char*)slide, "%3d%%", (ratio(G.crec.ucsize, csiz) + 5) / 10));
    }
    else if (uO.lflag == 5)
        Info(slide, 0, ((char*)slide, " %s", FmZofft(G.crec.csize, "8", "u")));
...
#endif
    Info(slide, 0, ((char*)slide, " %s %s ", methbuf, zi_time(__G__ & G.crec.last_mod_dos_datetime, z_modtim, d_t_buf)));
    fnprint(__G);
...
static void zi_showMacTypeCreator(__G__ ebfield) __GDEF uch* ebfield;
{
    ...
    if (isprint(native(ebfield[0])) && isprint(native(ebfield[1])) && isprint(native(ebfield[2])) && isprint(native(ebfield[3])) && isprint(native(ebfield[4])) && isprint(native(ebfield[5])) &&
        isprint(native(ebfield[6])) && isprint(native(ebfield[7]))) {
        Info(slide, 0,
             ((char*)slide, LoadFarString(MacOSdata), native(ebfield[0]), native(ebfield[1]), native(ebfield[2]), native(ebfield[3]), native(ebfield[4]), native(ebfield[5]), native(ebfield[6]),
              native(ebfield[7])));
    }
    else {
        Info(slide, 0,
             ((char*)slide, LoadFarString(MacOSdata1), (((ulg)ebfield[0]) << 24) + (((ulg)ebfield[1]) << 16) + (((ulg)ebfield[2]) << 8) + ((ulg)ebfield[3]),
              (((ulg)ebfield[4]) << 24) + (((ulg)ebfield[5]) << 16) + (((ulg)ebfield[6]) << 8) + ((ulg)ebfield[7])));
    }
...
char* d_t_str;
{
    ...
    static ZCONST char Far month[12][4] = {"Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"};
#ifdef USE_EF_UT_TIME
    struct tm* t;
#endif
    ...
    if (uO.lflag > 9) /* verbose listing format */
        sprintf(d_t_str, LoadFarString(lngYMDHMSTime), yr + 1900, monthstr, dy, hh, mm, ss);
    else if (uO.T_flag)
        sprintf(d_t_str, LoadFarString(DecimalTime), yr + 1900, mo, dy, hh, mm, ss);
    else /* was:  if ((uO.lflag >= 3) && (uO.lflag <= 5)) */
        sprintf(d_t_str, LoadFarString(shtYMDHMTime), yr % 100, monthstr, dy, hh, mm);

...
#define CHARS2LONG(b) (((ulg)(b)[0] << 24) | ((ulg)(b)[1] << 16) | ((ulg)(b)[2] << 8) | ((ulg)(b)[3]))
/* b must be (uch *) in each of these.  Now the reverse: */
#define LONG2CHARS(b, l) ((b)[0] = (uch)((l) >> 24), (b)[1] = (uch)((l) >> 16), (b)[2] = (uch)((l) >> 8), (b)[3] = (uch)(l))

#define COPYBUFFER 16384

ulg totalwritten = 0;

bool CopyData(FILE* out, FILE* inn, ulg archivesize, char* outname, char* inname) {
    ...
    do {
        ...
        if (fwrite(buf, 1, chunk, out) < chunk) {
            printf("Error writing %s to %s\n", archivesize ? "archive data" : "self-extractor code", outname);
            return false;
        }
        written += chunk;
        totalwritten += chunk;
...
void main(int argc, char** argv) {
    ...
    if (argc < 3 || argc > 4) {
        printf(
            "Usage: %s <result-file> <zip-archive> [<self-extractor-"
    ...
    ret = CopyData(out, tool, 0, argv[1], toolname) && CopyData(out, arch, archivesize, argv[1], argv[2]) ? 0 : 10;
    fclose(out);
    fclose(arch);
    fclose(tool);
    if (ret) {

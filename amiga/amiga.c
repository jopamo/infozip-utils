...
char* do_wild(__G__ wildspec) __GDEF ZCONST char* wildspec; /* only used first time on a given dir */
{
    ...
    */
    ...
    if (!G.notfirstcall) { /* first call:  must initialize everything */
        ...
        if (!ispattern(wildspec) || (lok = Lock((char*)wildspec, ACCESS_READ))) {
            if (lok)
                UnLock(lok);  /* ^^ we ignore wildcard chars if */
        ...
        if ((G.wildname = (ZCONST char*)strrchr(wildspec, '/')) == NULL && (G.wildname = (ZCONST char*)strrchr(wildspec, ':')) == NULL) {
            ...
            if ((G.dirname = (char*)malloc(G.dirnamelen + 1)) == NULL) {
                Info(slide, 1, ((char*)slide, "warning:  cannot allocate wildcard buffers\n"));
                strncpy(G.matchname, wildspec, FILNAMSIZ);
                G.matchname[FILNAMSIZ - 1] = '\0';
    ...
    switch (G.pInfo->hostnum) {
        ...
        case TANDEM_: {
            ...
            if (!r) {
                ...
                G.pInfo->file_attr = (unsigned)(uxattr & S_IWRITE ? uxattr | S_IDELETE : uxattr);
                break;
            }
        }
            /* fall through! */
 ...
 */
{
    ...
    G.renamed_fullpath = (renamed && (*G.filename == '/' || *G.filename == ':'));
#else
    /* supress G.rootpath even when user gave a relative pathname */
#if 1
    G.renamed_fullpath = (renamed && strpbrk(G.filename, ":/");
#else
    G.renamed_fullpath = (renamed && (strchr(G.filename, ':') || strchr(G.filename, '/')));
#endif
#endif
...
#define FUNCTION (flag & FN_MASK)
    ...
    if (FUNCTION == INIT) {
        ...
        if ((G.buildpath = (char*)malloc(strlen(G.filename) + G.rootlen + 1)) == (char*)NULL)
            return MPN_NOMEM;
...
void close_outfile(__G) __GDEF {
    ...
    if (uO.D_flag <= 1) {
        ...
        if (!FileDate(G.filename, &m_time))
            Info(slide, 1, ((char*)slide, "warning:  cannot set the time for %s\n", G.filename));
    }
...
int screensize(int* ttrows, int* ttcols) {
    ...
    if (fh && IsInteractive(fh)) {
        ...
        if (conunit) {
            if (ttrows)
                *ttrows = conunit->cu_YMax + 1;
            if (ttcols)
                *ttcols = conunit->cu_XMax + 1;
            return 0; /* success */
        }
    }
    if (ttrows)
...
BOOL is_floppy(ZCONST char* path) {
    ...
    struct DeviceNode *debb, devlist = (void*)BADDR((struct DosInfo*)BADDR(DOSBase->dl_Root->rn_Info)->di_DevInfo);
    if (!lok)
    ...
    for (debb = devlist; debb; debb = BADDR(debb->dn_Next))
        if (debb->dn_Type == DLT_DEVICE && (debb->dn_Task == lick->fl_Task))
            if (fart = BADDR(debb->dn_Startup)) {
                ...
                devname[i] = 0;
                okay = !strcmp(devname, "trackdisk.device") || !strcmp(devname, "mfm.device") || !strcmp(devname, "messydisk.device");
...
void version(__G) __GDEF {
    ...
    ******/
    ...
    printf(LoadFarString(CompiledWith), buf1, buf2, buf3, buf4, "", /* buf5 not used */
           "");                                                     /* buf6 not used */

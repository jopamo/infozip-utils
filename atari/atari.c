...
void close_outfile(__G) /* GRR: change to return PK-style warning level */
    ...
    if (G.symlnk) {
         ...
         */
        extent slnk_entrysize = (sizeof(slinkentry) + 1) + ucsize + strlen(G.filename);
    ...
    if (uO.D_flag <= 1) {
        ...
        eb_izux_flg = (G.extra_field
                               ...
                               && G.tz_is_valid
#endif
                           ? ef_scan_for_izux(G.extra_field, G.lrec.extra_field_length, 0, G.lrec.last_mod_dos_datetime, &zt, NULL)
                           : 0);
        ...
        if (utime(G.filename, &tp))
            Info(slide, 0x201, ((char*)slide, "warning:  cannot set the time for %s\n", FnFilter1(G.filename)));
    }

...
static int has_NFS_ext(const char* name) {
    ...
    return (i >= 0 && name[i] == ',' && (i > 0 || name[i - 1] == '/') && isxdigit(name[i + 1]) && isxdigit(name[i + 2]) && isxdigit(name[i + 3]));
...
static int mimemap(const char* name) {
    ...
    if (has_mimemap < 0)
        has_mimemap = !(SWI_OS_CLI("%RMEnsure MimeMap 0.05 RMLoad System:Modules.Network.MimeMap") || SWI_OS_CLI("%RMEnsure MimeMap 0.05"));
...
int mkdir(path, mode) const char* path;
int mode; /* ignored */
/*
...
void close_outfile(__G) __GDEF {
    ...
    if ((spark_ef = getRISCOSexfield(G.extra_field, G.lrec.extra_field_length)) != NULL) {
        ...
        loadaddr = (loadaddr & 0xfff000ffU) | ((G.pInfo->file_attr & 0xfff00000) >> 12);

        attr = (attr & 0xffffff00) | ((mode & 0400) >> 8) | ((mode & 0200) >> 6) | ((mode & 0004) << 2) | ((mode & 0002) << 4);
...
time_t modtime;
{
    ...
    /* set the file's modification time */
    if (SWI_OS_File_5((char*)fname, NULL, &loadaddr, NULL, NULL, &attr) != NULL)
        return -1;

    if (uxtime2acornftime(&execaddr, &loadaddr, modtime) != 0)
        return -1;

    return (SWI_OS_File_1((char*)fname, loadaddr, execaddr, attr) == NULL) ? 0 : -1;

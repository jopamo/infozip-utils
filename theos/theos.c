...
#define FUNCTION (flag & FN_MASK)
...
#endif
                {
                    size = (ulg)G.extra_field[5] | ((ulg)G.extra_field[6] << 8) | ((ulg)G.extra_field[7] << 16) | ((ulg)G.extra_field[8] << 24);
                }
...
#ifdef ACORN_FTYPE_NFS
        if ((buildpath = (char*)malloc(strlen(G.filename) + rootlen + (uO.acorn_nfs_ext ? 5 : 1)))
#else
    ...
    if (FUNCTION == ROOT) {
        ...
        if ((rootlen = strlen(pathcomp)) > 0) {
            ...
            if (rootlen > 0 && (SSTAT(tmproot, &G.statbuf) || !S_ISDIR(G.statbuf.st_mode))) { /* path does not exist */
...
static int get_extattribs(__G__ pzt) __GDEF iztimes* pzt;
{
/*---------------------------------------------------------------------------
...
void close_outfile(__G) /* GRR: change to return PK-style warning level */
    ...
    if (isv2_3()) {
        ...
        chlen(G.filename, (ush)G.extra_field[10] | ((ush)G.extra_field[11] << 8), (ush)G.extra_field[12] | ((ush)G.extra_field[13] << 8));
...
static ZCONST char Far DirlistUidGidFailed[] = "warning:  cannot set UID %d and/or GID %d for %s\n";
...
void version(__G) __GDEF {
    ...
    sprintf(slide, CompiledWith, "THEOS C ", "5.28", "THEOS ", "4.x", " on ", __DATE__);

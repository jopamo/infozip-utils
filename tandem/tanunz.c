...
static nsk_file_attrs* ef_scan_for_tandem(uch* ef_buf, unsigned ef_len);
...
unsigned ef_len; /* total length of extra field */
{
    ...
    while (ef_len >= EB_HEADSIZE) {
        ...
        if (eb_len > (ef_len - EB_HEADSIZE)) {
            ...
            TTrace((stderr, "ef_scan_for_tandem: block length %u > rest ef_size %u\n", eb_len, ef_len - EB_HEADSIZE));
            break;
        }
...
#define vlist_bytes 2
    ...
    if (SSTAT(G.filename, &G.statbuf) == 0) {
        ...
        if (unlink(G.filename) != 0) {
            ...
            if (unlink(G.filename) != 0) {
                Info(slide, 0x401, ((char*)slide, LoadFarString(CannotDeleteOldFile), FnFilter1(G.filename)));
                return 1;
            }
        }
    ...
    if (znsk_attr != NULL) {
        ...
        filecode = (znsk_attr->filetype != NSK_UNSTRUCTURED ? (G.pInfo->textmode ? NSK_EDITFILECODE : NSK_UNSTRUCTURED) : znsk_attr->filecode);
    ...
    if ((fdesc = creat(G.filename, , priext, secext)) != -1) {
        ...
        err = STRING_UPSHIFT_(G.filename, len, nsk_fname, len);

        err = FILE_ALTERLIST_(nsk_fname, len, alist, alist_items, vlist, vlist_bytes, , err_item);
    };

    G.outfile = fopen(G.filename, (G.pInfo->textmode ? FOPWT : FOPW));

    if (G.outfile == (FILE*)NULL) {
        Info(slide, 1, ((char*)slide, LoadFarString(CannotCreateFile), FnFilter1(G.filename)));
        return 1;
    }
...
#define FUNCTION (flag & FN_MASK)
    ...
    if (FUNCTION == INIT) {
        ...
        if ((buildpath = (char*)malloc(strlen(G.filename) + rootlen + 1)) == (char*)NULL)
            return MPN_NOMEM;
...
int mkdir(path, mode) const char* path; /* both    */
mode_t mode;                            /* ignored */
/*
...
#define vlist_bytes 2
    ...
    if (znsk_attr != NULL) {
        /* Reset File Code */
        if (!uO.bflag && !G.pInfo->textmode && znsk_attr->filetype == NSK_UNSTRUCTURED) {
...
void version(__G) __GDEF {
    ...
    sprintf((char*)slide, LoadFarString(CompiledWith),
            ...
            "C ", "T9255D44 - (16OCT98)", "NonStop ", "(Tandem/NSK)",

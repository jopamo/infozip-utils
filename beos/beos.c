...
static ZCONST char CannotSetItemUidGid[] = "warning:  cannot set UID %lu and/or GID %lu for %s\n          %s\n";
static ZCONST char CannotSetUidGid[] = " (warning) cannot set UID %lu and/or GID %lu\n          %s";
static ZCONST char CannotSetItemTimestamps[] = "warning:  cannot set modif./access times for %s\n          %s\n";
    ...
    if (!G.notfirstcall) { /* first call:  must initialize everything */
        ...
        if ((G.wildname = (ZCONST char*)strrchr(wildspec, '/')) == NULL) {
            ...
            if ((G.dirname = (char*)malloc(G.dirnamelen + 1)) == (char*)NULL) {
                Info(slide, 0x201, ((char*)slide, "warning:  cannot allocate wildcard buffers\n"));
                strncpy(G.matchname, wildspec, FILNAMSIZ);
                G.matchname[FILNAMSIZ - 1] = '\0';
    ...
    switch (G.pInfo->hostnum) {
        ...
        case TANDEM_:
                    ...
                    switch (ebID) {
                        case EF_ASIUNIX:
                            if (ebLen >= (EB_ASI_MODE + 2)) {
                                G.pInfo->file_attr = (unsigned)makeword(ef + (EB_HEADSIZE + EB_ASI_MODE));
                                /* force stop of loop: */
            ...
            if (!r) {
                 ...
                 */
                G.pInfo->symlink = S_ISLNK(G.pInfo->file_attr) && SYMLINK_HOST(G.pInfo->hostnum);
#endif
                return 0;
            }
            /* fall through! */
        ...
        default:
            ...
            if ((G.pInfo->file_attr & 0700) == (unsigned)(0400 | tmp << 6)) {
                 ...
                 */
                G.pInfo->symlink = S_ISLNK(G.pInfo->file_attr) && (G.pInfo->hostnum == FS_FAT_);
#endif
                return 0;
            }
 ...
 */
{
    ...
    if (G.filename[strlen(G.filename) - 1] == '/') {
        ...
        if (G.created_dir) {
            ...
            if (!uO.J_flag) { /* Handle the BeOS extra field if present. */
                void* ptr = scanBeOSexfield(G.extra_field, G.lrec.extra_field_length);
                if (ptr) {
    ...
    *pp = '\0'; /* done with pathcomp:  terminate it */
    ...
    if (uO.acorn_nfs_ext && (ef_spark = (RO_extra_block*)getRISCOSexfield(G.extra_field, G.lrec.extra_field_length)) != (RO_extra_block*)NULL) {
        /* file *must* have a RISC OS extra field */
        long ft = (long)makelong(ef_spark->loadaddr);
        /*32-bit*/
        if (lastcomma) {
            pp = lastcomma + 1;
            while (isxdigit((uch)(*pp)))
                ++pp;
            if (pp == lastcomma + 4 && *pp == '\0')
                *lastcomma = '\0'; /* nuke */
        }
        if ((ft & 1 << 31) == 0)
            ft = 0x000FFD00;
...
#define FUNCTION (flag & FN_MASK)
    ...
    if (FUNCTION == APPEND_DIR) {
        ...
        if (SSTAT(G.buildpath, &G.statbuf)) { /* path doesn't exist */
            ...
            if (too_long) {
                Info(slide, 1, ((char*)slide, "checkdir error:  path too long: %s\n", FnFilter1(G.buildpath)));
                free(G.buildpath);
            ...
            if (mkdir(G.buildpath, 0777) == -1) { /* create the directory */
                Info(slide, 1,
                 ...
                 %s\n\
                 unable to process %s.\n",
                      FnFilter2(G.buildpath), strerror(errno), FnFilter1(G.filename)));
                free(G.buildpath);
...
#ifdef ACORN_FTYPE_NFS
        if ((G.buildpath = (char*)malloc(strlen(G.filename) + G.rootlen + (uO.acorn_nfs_ext ? 5 : 1)))
#else
    ...
    if (FUNCTION == ROOT) {
        ...
        if ((G.rootlen = strlen(pathcomp)) > 0) {
            ...
            if (G.rootlen > 0 && (SSTAT(tmproot, &G.statbuf) || !S_ISDIR(G.statbuf.st_mode))) { /* path does not exist */
...
ulg z_uidgid[2];
{
    ...
    eb_izux_flg = (G.extra_field ? ef_scan_for_izux(G.extra_field, G.lrec.extra_field_length, 0, G.lrec.last_mod_dos_datetime,
#ifdef IZ_CHECK_TZ
...
void close_outfile(__G) /* GRR: change to return PK-style warning level */
    ...
    if (G.symlnk) {
        ...
        if (!uO.J_flag) {
            ...
            BeOS_exfld = scanBeOSexfield(G.extra_field, G.lrec.extra_field_length);
            if (BeOS_exfld) {
         ...
         */
        slnk_entrysize = (sizeof(slinkentry) + 1) + BeOSef_len + ucsize + strlen(G.filename);
    ...
    if (have_uidgid_flg
        ...
        && ((ulg)(uid_t)(z_uidgid[0]) == z_uidgid[0]) && ((ulg)(gid_t)(z_uidgid[1]) == z_uidgid[1])) {
        TTrace((stderr, "close_outfile:  restoring Unix UID/GID info\n"));
        if (chown(G.filename, (uid_t)z_uidgid[0], (gid_t)z_uidgid[1])) {
            if (uO.qflag)
                Info(slide, 0x201, ((char*)slide, CannotSetItemUidGid, z_uidgid[0], z_uidgid[1], FnFilter1(G.filename), strerror(errno)));
            else
                Info(slide, 0x201, ((char*)slide, CannotSetUidGid, z_uidgid[0], z_uidgid[1], strerror(errno)));
        }
    }
    ...
    if (uO.D_flag <= 1) {
        ...
        if (utime(G.filename, &(zt.t2))) {
            if (uO.qflag)
                Info(slide, 0x201, ((char*)slide, CannotSetItemTimestamps, FnFilter1(G.filename), strerror(errno)));
            else
                Info(slide, 0x201, ((char*)slide, CannotSetTimestamps, strerror(errno)));
        }
    }
...
#ifndef NO_CHMOD
static ZCONST char DirlistChmodFailed[] = "warning:  cannot set permissions for %s\n          %s\n";
#endif
...
int defer_dir_attribs(__G__ pd) __GDEF direntry** pd;
{
    ...
    *pd = (direntry*)d_entry;
    ...
    d_entry->have_uidgid = get_extattribs(__G__ & (d_entry->u.t3), d_entry->uidgid);
    return PK_OK;
...
int set_direc_attribs(__G__ d) __GDEF direntry* d;
{
    ...
    if (UxAtt(d)->have_uidgid &&
        /* check that both uid and gid values fit into their data sizes */
        ((ulg)(uid_t)(UxAtt(d)->uidgid[0]) == UxAtt(d)->uidgid[0]) && ((ulg)(gid_t)(UxAtt(d)->uidgid[1]) == UxAtt(d)->uidgid[1]) &&
        chown(UxAtt(d)->fn, (uid_t)UxAtt(d)->uidgid[0], (gid_t)UxAtt(d)->uidgid[1])) {
        Info(slide, 0x201, ((char*)slide, CannotSetItemUidGid, UxAtt(d)->uidgid[0], UxAtt(d)->uidgid[1], FnFilter1(d->fn), strerror(errno)));
        if (!errval)
            errval = PK_WARN;
    }
    ...
    if (uO.D_flag <= 0) {
        ...
        if (utime(d->fn, (const struct utimbuf*)&UxAtt(d)->u.t2)) {
            Info(slide, 0x201, ((char*)slide, CannotSetItemTimestamps, FnFilter1(d->fn), strerror(errno)));
            if (!errval)
                errval = PK_WARN;
        }
    }
    ...
    if (chmod(d->fn, filtattr(__G__ UxAtt(d)->perms))) {
        Info(slide, 0x201, ((char*)slide, DirlistChmodFailed, FnFilter1(d->fn), strerror(errno)));
        if (!errval)
            errval = PK_WARN;
    }
#endif /* !NO_CHMOD */
...
static uch* scanBeOSexfield(const uch* ef_ptr, unsigned ef_len) {
    while (ef_ptr != NULL && ef_len >= EB_HEADSIZE) {
        ...
        if (eb_len > (ef_len - EB_HEADSIZE)) {
            Trace((stderr, "scanBeOSexfield: block length %u > rest ef_size %u\n", eb_len, ef_len - EB_HEADSIZE));
            break;
        }
...
*/
static int set_file_attrs(const char* name, const unsigned char* attr_buff, const off_t attr_size) {
    int retval = EOK;
    unsigned char* ptr;
    ...
    while (ptr < guard) {
        ...
        (void)swap_data(fa_info.type, attr_data, fa_info.size, B_SWAP_BENDIAN_TO_HOST);

        wrote_bytes = fs_write_attr(fd, attr_name, fa_info.type, 0, attr_data, fa_info.size);
        if (wrote_bytes != fa_info.size) {
            Info(slide, 0x201, ((char*)slide, "warning: wrote %ld attribute bytes of %ld\n", (unsigned long)wrote_bytes, (unsigned long)fa_info.size));
        }
    }
...
static void setBeOSexfield(const char* path, uch* extra_field) {
    ...
    id = makeword(ptr);
    ptr += 2; /* we don't use this... */
    size = makeword(ptr);
    ptr += 2;
    full_size = makelong(ptr);
    ptr += 4;
    flags = *ptr;
    ptr++;
    ...
    if (flags & EB_BE_FL_BADBITS) {
        ...
        Info(slide, 0x201, ((char*)slide, "Unsupported flags set for this BeOS extra field, skipping.\n"));
        return;
    }
    if (size <= EB_BEOS_HLEN) {
        /* corrupted, unsupported, or truncated */
        Info(slide, 0x201, ((char*)slide, "BeOS extra field is %d bytes, should be at least %d.\n", size, EB_BEOS_HLEN));
        return;
    }

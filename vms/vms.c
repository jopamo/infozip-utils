 ...
 */
ZCONST char Far InvalidResponse[] = "error:  invalid response [%.1s]\n";
...
static int (*_flush_routine)(__GPRO__ uch* rawbuf, unsigned size, int final_flag);
...
unsigned char char_prop[256] = {
    ...
    64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16,
    ...
    16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 64};
...
#define PRINTABLE_FORMAT(x) ((x) == FAB$C_VAR || (x) == FAB$C_STMLF || (x) == FAB$C_STMCR || (x) == FAB$C_STM)
...
static ZCONST char* month[] = {"JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DEC"};

/*   buffer for time string */
static char timbuf[24]; /* length = first entry in "date_str" + 1 */

/*   fixed-length string descriptor for timbuf: */
static ZCONST struct dsc$descriptor date_str = {sizeof(timbuf) - 1, DSC$K_DTYPE_T, DSC$K_CLASS_S, timbuf};
...
static void set_default_datetime_XABs(__GPRO) {
    ...
    if (G.extra_field &&
        ...
        G.tz_is_valid &&
#endif
        (ef_scan_for_izux(G.extra_field, G.lrec.extra_field_length, 0, G.lrec.last_mod_dos_datetime, &z_utime, NULL) & EB_UT_FL_MTIME))
    ...
    rdt = cc$rms_xabrdt;
    sprintf(timbuf, "%02u-%3s-%04u %02u:%02u:%02u.00", dy, month[mo], yr, hh, mm, ss);
...
static int create_default_output(__GPRO) {
    ...
    text_output = (G.pInfo->textmode
                  ...
                  (uO.cflag && (!uO.bflag || (!(uO.bflag - 1) && G.pInfo->textfile)));
    ...
    if (!uO.cflag) /* Redirect output */
    {
        ...
        if (text_output && (!uO.S_flag)) { /* Default format for output `real' text file */
            fileblk.fab$b_rfm = FAB$C_VAR; /* variable length records */
            fileblk.fab$b_rat = FAB$M_CR;  /* implied (CR) carriage ctrl */
        }
        else if (bin_fixed) {              /* Default format for output `real' binary file */
        ...
        else {                               /* Default format for output misc (bin or text) file */
        ...
        if (ierr == RMS$_FEX) {
            ...
            switch (ierr & REPL_TASKMASK) {
                case REPL_NO_EXTRACT: /* No extract. */
                    free_up();
                    return ((ierr & REPL_ERRLV_WARN) ? OPENOUT_SKIPWARN : OPENOUT_SKIPOK);
...
#endif
            Info(slide, 1, ((char*)slide, "Cannot create ($connect) output file:  %s\n", FnFilter1(G.filename)));
            free_up();
            return OPENOUT_FAILED;
        }
    } /* end if (!uO.cflag) */
...
static int create_rms_output(__GPRO) {
     ...
     */
    text_output = uO.cflag && (!uO.bflag || (!(uO.bflag - 1) && G.pInfo->textfile));
    ...
    if (uO.cflag) /* SYS$OUTPUT */
    {
        if (text_output && !PRINTABLE_FORMAT(rfm)) {
            Info(slide, 1, ((char*)slide, "[ File %s has illegal record format to put to screen ]\n", FnFilter1(G.filename)));
            free_up();
            return OPENOUT_FAILED;
        }
    }
...
static struct dsc$descriptor pka_fibdsc = {sizeof(pka_fib), DSC$K_DTYPE_Z, DSC$K_CLASS_S, (void*)&pka_fib};

static struct dsc$descriptor_s pka_devdsc = {0, DSC$K_DTYPE_T, DSC$K_CLASS_S, &nam.NAM_DVI[1]};

static struct dsc$descriptor_s pka_fnam = {0, DSC$K_DTYPE_T, DSC$K_CLASS_S, NULL};
...
#define PK_PRINTABLE_RECTYP(x) ((x) == FAT$C_VARIABLE || (x) == FAT$C_STREAMLF || (x) == FAT$C_STREAMCR || (x) == FAT$C_STREAM)
...
#define SWAPW(x) ((((x) >> 16) & 0xFFFF) + ((x) << 16))
        ...
        if (status == SS$_DUPFILENAME) {
             ...
             */
            short res_nam_len;
            struct dsc$descriptor_s res_nam_dscr = {0, DSC$K_DTYPE_T, DSC$K_CLASS_S, NULL};
            ...
            switch (status & REPL_TASKMASK) {
                case REPL_NO_EXTRACT: /* No extract. */
                    free_up();
                    return ((status & REPL_ERRLV_WARN) ? OPENOUT_SKIPWARN : OPENOUT_SKIPOK);
                case REPL_NEW_VERSION: /* Create a new version. */
...
#define LINK(p)                                 \
    { /* Link xaballs and xabkeys into chain */ \
        if (first_xab == NULL)                  \
            first_xab = (void*)p;               \
        if (last_xab != NULL)                   \
    ...
    while (len > 0) {
        ...
        if (EQL_W(&hdr->tag, IZ_SIGNATURE)) {
            ...
            if (EQL_L(block_id, FABSIG)) {
                outfab = (struct FAB*)extract_izvms_block(__G__ blk, siz, NULL, (uch*)&cc$rms_fab, FABL);
            }
            else if (EQL_L(block_id, XALLSIG)) {
                xaball = (struct XABALL*)extract_izvms_block(__G__ blk, siz, NULL, (uch*)&cc$rms_xaball, XALLL);
                LINK(xaball);
            }
            else if (EQL_L(block_id, XKEYSIG)) {
                xabkey = (struct XABKEY*)extract_izvms_block(__G__ blk, siz, NULL, (uch*)&cc$rms_xabkey, XKEYL);
                LINK(xabkey);
            }
            else if (EQL_L(block_id, XFHCSIG)) {
                xabfhc = (struct XABFHC*)extract_izvms_block(__G__ blk, siz, NULL, (uch*)&cc$rms_xabfhc, XFHCL);
            }
            else if (EQL_L(block_id, XDATSIG)) {
                if (set_date_time) {
                    xabdat = (struct XABDAT*)extract_izvms_block(__G__ blk, siz, NULL, (uch*)&cc$rms_xabdat, XDATL);
                }
            }
            else if (EQL_L(block_id, XRDTSIG)) {
                if (set_date_time) {
                    xabrdt = (struct XABRDT*)extract_izvms_block(__G__ blk, siz, NULL, (uch*)&cc$rms_xabrdt, XRDTL);
                }
            }
            else if (EQL_L(block_id, XPROSIG)) {
                xabpro = (struct XABPRO*)extract_izvms_block(__G__ blk, siz, NULL, (uch*)&cc$rms_xabpro, XPROL);
            }
                ...
                vers = extract_izvms_block(__G__ blk, siz, &verlen, NULL, 0);
                if ((m = strrchr((char*)vers, '-')) != NULL)
                    *m = '\0'; /* Cut out release number */
                if (strcmp(verbuf, (char*)vers) && uO.qflag < 2) {
                    Info(slide, 0, ((char*)slide, "[ Warning: VMS version mismatch."));

                    Info(slide, 0, ((char*)slide, "   This version %s --", verbuf));
                    strncpy(verbuf, (char*)vers, verlen);
                    verbuf[verlen] = '\0';
                    Info(slide, 0, ((char*)slide, " version made by %s ]\n", verbuf));
                }
                free(vers);
#endif /* CHECK_VERSIONS */
            }
            else {
                Info(slide, 1, ((char*)slide, "[ Warning: Unknown block signature %s ]\n", block_id));
            }
        }
...
int final_flag; /* 1 if this is the final flushout */
{
    ...
    while (size > 0) {
        if (curbuf->bufcnt < BUFS512) {
            ...
            ncpy = size > (BUFS512 - curbuf->bufcnt) ? (BUFS512 - curbuf->bufcnt) : size;
    ...
    return (final_flag && (curbuf->bufcnt > 0)) ? WriteBuffer(__G__ curbuf->buf, curbuf->bufcnt) : PK_COOL;
}
...
unsigned len;
{
     ...
     */
    status = sys$qio(0, pka_devchn, IO$_WRITEVBLK, &pka_io_iosb, 0, 0, buf, len, pka_vbn, 0, 0, 0);
    if (ERR(status)) {
...
int final_flag; /* 1 if this is the final flushout */
{
    ...
    if (final_flag) {
        if (loccnt > 0) {
            ...
            status = sys$qiow(0, pka_devchn, IO$_WRITEVBLK, &pka_io_iosb, 0, 0, locbuf, loccnt_even, pka_vbn, 0, 0, 0);
            if (!ERR(status))
                status = pka_io_iosb.status;
            if (ERR(status)) {
    ...
    if (loccnt > 0) {
        ...
        out_ptr += ncpy;
        if (loccnt == 512) {
            status = sys$qiow(0, pka_devchn, IO$_WRITEVBLK, &pka_io_iosb, 0, 0, locbuf, loccnt, pka_vbn, 0, 0, 0);
            if (!ERR(status))
                status = pka_io_iosb.status;
            if (ERR(status)) {
                vms_msg(__G__ "[ Write QIO failed ]\n", status);
    ...
    if (size >= 512) {
        ...
        status = sys$qiow(0, pka_devchn, IO$_WRITEVBLK, &pka_io_iosb, 0, 0, out_ptr, put_cnt, pka_vbn, 0, 0, 0);
        if (!ERR(status))
            status = pka_io_iosb.status;
        if (ERR(status)) {
            vms_msg(__G__ "[ Write QIO failed ]\n", status);
...
int final_flag;
{
    ...
    if (loccnt > 0) /* incomplete record left from previous call */
    {
        ...
        if (nneed > size) {
            if (size + loccnt > BUFSMAXREC) {
                char buf[80];
                Info(buf, 1, (buf, "[ Record too long (%u bytes) ]\n", reclen));
                return PK_DISK;
            }
    ...
    if (final_flag && loccnt > 0) {
        ...
        Info(buf, 1, (buf, "[ Warning, incomplete record of length %u ]\n", (unsigned)*(ush*)locbuf));
...
#ifdef USE_ORIG_DOS
#define ORG_DOS (G.pInfo->hostnum == FS_FAT_ || G.pInfo->hostnum == FS_HPFS_ || G.pInfo->hostnum == FS_NTFS_)
#else
#define ORG_DOS 1
#endif
...
#define RECORD_END(c, f)                                                                                                                               \
    ((ORG_DOS || G.pInfo->textmode) && c == CTRLZ || (f == FAB$C_STMLF && c == LF) || (f == FAB$C_STMCR || ORG_DOS || G.pInfo->textmode) && c == CR || \
    ...
    if (loccnt > 0) {
        ...
        if (got_eol) {
            ...
            if (loccnt + eol_off > BUFSMAXREC) {
                ...
                recsize = loccnt;
                start = 0;
                Info(buf, 1, (buf, "[ Warning: Record too long (%u) ]\n", loccnt + eol_off));
                complete = 1;
                end = 0;
            }
            else {
    ...
    if (G.symlnk) {
        ...
        if (G.pInfo->symlink == 0) {
            if (QCOND2) {
                ...
                if (link_target == NULL) {
                    Info(slide, 0x201, ((char*)slide, "warning:  cannot show symlink (%s) target, no mem\n", FnFilter1(G.filename)));
                    retcode = PK_MEM;
                }
                else {
                    /* Read the link text. */
                    ...
                    if (ERR(status)) {
                        Info(slide, 0x201, ((char*)slide, "warning:  error reading symlink text: %s\n", strerror(EVMSERR, status)));
                        retcode = PK_DISK;
                    }
                    else {
                        Info(slide, 0, ((char*)slide, "-> %s ", FnFilter1(link_target)));
                    }
             ...
             */
            slnk_entrysize = (sizeof(slinkentry) + 1) + ucsize + strlen(G.filename);

            if (slnk_entrysize < ucsize) {
                Info(slide, 0x201, ((char*)slide, "warning:  symbolic link (%s) failed: mem alloc overflow\n", FnFilter1(G.filename)));
                retcode = PK_ERR;
            }
            else {
                if ((slnk_entry = (slinkentry*)malloc(slnk_entrysize)) == NULL) {
                    ...
                    if (ERR(status)) {
                        Info(slide, 0x201, ((char*)slide, "warning:  error reading symlink text (rms): %s\n", strerror(EVMSERR, status)));
                        free(slnk_entry);
                        retcode = PK_DISK;
                    }
                    else {
                        if (QCOND2)
                            Info(slide, 0, ((char*)slide, "-> %s ", FnFilter1(slnk_entry->target)));
...
#ifdef DEBUG
    if (ERR(status)) {
        vms_msg(__G__ "\r[ Warning: cannot set owner/protection/time attributes ]\n", status);
        if (outfab->fab$l_stv != 0) {
            vms_msg(__G__ "", outfab->fab$l_stv);
        }
        retcode = PK_WARN;
...
static int _close_qio(__GPRO) {
    ...
    pka_fib.FIB$L_ACCTL = FIB$M_WRITE | FIB$M_NOTRUNC;
    pka_fib.FIB$W_EXCTL = 0;

    pka_fib.FIB$W_FID[0] = pka_fib.FIB$W_FID[1] = pka_fib.FIB$W_FID[2] = pka_fib.FIB$W_DID[0] = pka_fib.FIB$W_DID[1] = pka_fib.FIB$W_DID[2] = 0;
    ...
    if (G.symlnk && QCOND2) {
        ...
        if ((link_target = malloc(ucsize + 1)) == NULL) {
            ...
            if (ERR(status)) {
                Info(slide, 0x201, ((char*)slide, "warning:  error reading symlink text (qio): %s\n", strerror(EVMSERR, status)));
            }
            else {
                Info(slide, 0, ((char*)slide, "-> %s ", FnFilter1(link_target)));
            }
    ...
    status = sys$qiow(0, pka_devchn, IO$_DEACCESS, &pka_acp_iosb, 0, 0, &pka_fibdsc, 0, 0, 0, pka_atr, 0);
...
static char* vms_path_fixdown(ZCONST char* dir_spec, char* dir_file) {
    ...
    while ((i > 0) && ((dir_spec[i - 1] == '^') || ((dir_spec[i] != '.') && (dir_spec[i] != dir_open)))) {
        i--;
    }
...
int set_direc_attribs(__G__ d) __GDEF direntry* d;
{
    ...
    if (type != VAT_NONE) {
        if (type == VAT_IZ) {
            ...
            if (xabpro != NULL) {
                ...
                attr = VmsAtt(d)->perms | (((1 << XAB$V_NODEL) << XAB$V_SYS) | ((1 << XAB$V_NODEL) << XAB$V_OWN) | ((1 << XAB$V_NODEL) << XAB$V_GRP) | ((1 << XAB$V_NODEL) << XAB$V_WLD));
            }
        ...
        /* Revoke directory Delete permission for all. */
        attr |= (((1 << XAB$V_NODEL) << XAB$V_SYS) | ((1 << XAB$V_NODEL) << XAB$V_OWN) | ((1 << XAB$V_NODEL) << XAB$V_GRP) | ((1 << XAB$V_NODEL) << XAB$V_WLD));
    ...
    if (ERR(status)) {
        sprintf(warnmsg, "warning:  set-dir-attributes failed ($parse) for %s.\n", dir_name);
    ...
    if (ERR(status)) {
        sprintf(warnmsg, "warning:  set-dir-attributes failed ($assign) for %s.\n", dir_name);
        vms_msg(__G__ warnmsg, status);
        retcode = PK_WARN;
        goto cleanup_exit;
    }
    ...
    */
    ...
    for (i = 0; pka_atr[i].atr$w_type != 0; i++) {
        if (pka_atr[i].atr$w_type == ATR$C_RECATTR) {
            ...
            if (ERR(status)) {
                sprintf(warnmsg, "warning:  set-dir-attributes failed ($qiow acc) for %s.\n", dir_name);
                vms_msg(__G__ warnmsg, status);
                retcode = PK_WARN;
            }
            else {
    ...
    if (ERR(status)) {
        sprintf(warnmsg, "warning:  set-dir-attributes failed ($qiow mod) for %s.\n", dir_name);
        vms_msg(__G__ warnmsg, status);
        retcode = PK_WARN;
    }
    sys$dassgn(pka_devchn);
...
static ZCONST short ydays[] = {0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334, 365};
...
struct tm* tm;
{
    ...
    dy = (unsigned)dy + ((unsigned)yr * 365) + (unsigned)nleap(yr + 1970) + (unsigned)ydays[mo] + ((mo > 1) && leap(yr + 1970));

    /* convert date & time to seconds relative to 00:00:00, 01/01/1970 */
    return (time_t)((unsigned long)(unsigned)dy * 86400L + (unsigned long)hh * 3600L + (unsigned long)(mm * 60 + ss));
...
ulg dosdatetime;
{
    ...
    if ((dosdatetime >= DOSTIME_2038_01_18) && (then < (time_t)0x70000000L))
...
static void uxtime2vmstime(/* convert time_t value into 64 bit VMS bintime */
    ...
    sprintf(timbuf, "%02d-%3s-%04d %02d:%02d:%02d.00", t->tm_mday, month[t->tm_mon], t->tm_year + 1900, t->tm_hour, t->tm_min, t->tm_sec);
...
#pragma __member_alignment __restore
    ...
    static struct atrdef Atr[] = {{sizeof(pka_rattr), ATR$C_RECATTR, GVTC & pka_rattr},
    ...
    */
    status = sys$qiow(0, pka_devchn, IO$_ACCESS, &pka_acp_iosb, 0, 0, &pka_fibdsc, &pka_fnam, 0, 0, Atr, 0);
    ...
    */
    ...
    status = sys$qiow(0, pka_devchn, IO$_MODIFY, &pka_acp_iosb, 0, 0, &pka_fibdsc, &pka_fnam, 0, 0, Atr, 0);
...
static void vms_msg(__GPRO__ ZCONST char* string, int status) {
    if (ERR(vms_msg_fetch(status)))
        Info(slide, 1, ((char*)slide, "%s[ VMS status = %d ]\n", string, status));
    else
 ...
 */
char* do_wild(__G__ wld) __GDEF ZCONST char* wld;
{
    int status;
    ...
    if (first_call || strcmp(wld, last_wild)) { /* (Re)Initialize everything */
...
int mapattr(__G) __GDEF {
    ...
    ulg theprot;
    static ulg defprot = (ulg)-1L, sysdef, owndef, grpdef, wlddef; /* Default protection fields */
    ...
    switch (G.pInfo->hostnum) {
        case AMIGA_:
            ...
            G.pInfo->file_attr = (tmp << XAB$V_OWN) | grpdef | sysdef | wlddef;
            break;
        ...
        case TANDEM_: {
                    ...
                    switch (ebID) {
                        case EF_ASIUNIX:
                            if (ebLen >= (EB_ASI_MODE + 2)) {
                                uxattr = (unsigned)makeword(ef + (EB_HEADSIZE + EB_ASI_MODE));
                                /* force stop of loop: */
            ...
            if (!r) {
                 ...
                 */
                G.pInfo->symlink = S_ISLNK(uxattr) && SYMLINK_HOST(G.pInfo->hostnum);
#endif
                theprot = (unix_to_vms[uxattr & 07] << XAB$V_WLD) | (unix_to_vms[(uxattr >> 3) & 07] << XAB$V_GRP) | (unix_to_vms[(uxattr >> 6) & 07] << XAB$V_OWN);
                if (uxattr & 0x4000)
                    /* Directory -- set D bits */
                    theprot |= (XAB$M_NODEL << XAB$V_SYS) | (XAB$M_NODEL << XAB$V_OWN) | (XAB$M_NODEL << XAB$V_GRP) | (XAB$M_NODEL << XAB$V_WLD);
        ...
        default:
            ...
            {            /* Bit is set -- set bits in all fields */
                ...
                theprot |= (tmp << XAB$V_SYS) | (tmp << XAB$V_OWN) | (tmp << XAB$V_GRP) | (tmp << XAB$V_WLD);
            }
            G.pInfo->file_attr = theprot;
            break;
...
static void adj_file_name_ods2(char* dest, char* src) {
    ...
    else {              /* Some kind of valid version. */
    ...
    __GDEF int renamed;
{
    ...
    */
    if (ods2_names < 0) {
        ...
        if (uO.ods2_flag == 0) {
            ods2_names = (dest_struct_level(PATH_DEFAULT) <= DVI$C_ACP_F11V2);
        }
        else {
    ...
    *pathcomp = '\0';                      /* Initialize translation buffer. */
    ...
    while (cp < last_slash) {
        ...
        if ((dir_len == 2) && (strncmp(cp, "..", 2) == 0)) { /* Double dot. */
        ...
        else if ((dir_len == 1) && (strncmp(cp, ".", 1) == 0)) { /* Single dot.  No-op.  Skip it. */
            dir_len = 0;
        }
    ...
    if (killed_ddot && QCOND2) {
        Info(slide, 0, ((char*)slide, "warning:  skipped \"../\" path component(s) in %s\n", FnFilter1(G.filename)));
        if (!(error & ~MPN_MASK))
...
int fcn;
{
    ...
    if (function == ROOT) { /*  Assume VMS root spec */
        /* 2006-01-20 SMS.
        ...
        */
        ...
        if (((status & STS$M_SEVERITY) != STS$K_SUCCESS) && (status != RMS$_DNF)) {
            /* Invalid destination directory specified. */
            Info(slide, 1, ((char*)slide, "Invalid destination directory (parse error): %s\n", FnFilter1(pathcomp)));
            return MPN_ERR_SKIP;
        }
        ...
        */
        if (nam.NAM_B_NAME + nam.NAM_B_TYPE + nam.NAM_B_VER > 2) {
            Info(slide, 1, ((char*)slide, "Invalid destination directory (includes file name): %s\n", FnFilter1(nam.NAM_ESA)));
            return MPN_ERR_SKIP;
        }
        ...
        *nam.NAM_L_NAME = '\0';
        ...
        if ((nam.NAM_ESA[rootlen - 1] == ']') && (nam.NAM_ESA[rootlen - 2] != '^')) {
            root_has_dir = 1;
            rootlen -= 2;
            while ((nam.NAM_ESA[rootlen] == '.') && (nam.NAM_ESA[rootlen - 1] != '^')) {
                rootlen--;
            }
            nam.NAM_ESA[++rootlen] = ']';
        ...
        if (status == RMS$_DNF) {
            if (status = mkdir(nam.NAM_ESA, 0)) {
                Info(slide, 1, ((char*)slide, "Can not create destination directory: %s\n", FnFilter1(nam.NAM_ESA)));
    ...
    if (function == APPEND_DIR) {
        ...
        else if (cmplen == 2 && pathcomp[0] == '.' && pathcomp[1] == '.') { /* ".../../..." -- convert to "...-..." */
            *end++ = '-';
            *end++ = '.';
        }
    ...
    if (function == APPEND_NAME) {
        if (fcn & USE_DEFAULT) { /* Expand renamed filename using collected path, return
                                  *  at pathcomp */
            fab = cc$rms_fab;    /* Initialize FAB. */
            nam = CC_RMS_NAM;    /* Initialize NAM[L]. */
            fab.FAB_NAM = &nam;  /* Point FAB to NAM[L]. */
            ...
            { /* ... try to create it */
                ...
                save = *dirend;
                *dirend = '\0';
                if ((mkdir_failed = mkdir(nam.NAM_L_DEV, 0)) && errno == EEXIST)
                    mkdir_failed = 0;
                *dirend = save;
                if (mkdir_failed)
                    return 3;
...
#ifdef RETURN_CODES
void return_VMS(__G__ err) __GDEF
#else
void return_VMS(err)
#endif
    int err;
...
#define OUTDEVICE_NAME "SYS$OUTPUT"
    ...
    static ZCONST struct dsc$descriptor_s OutDevDesc = {(sizeof(OUTDEVICE_NAME) - 1), DSC$K_DTYPE_T, DSC$K_CLASS_S, OUTDEVICE_NAME};
    ...
    if (OK(status)) {
         ...
         */
        status = sys$qiow(0, OutDevChan, IO$_SENSEMODE, &iosb, 0, 0, &ttmode, sizeof(ttmode), 0, 0, 0, 0);
    ...
    if (OK(status) && OK(status = iosb[0])) {
        if (tt_rows != NULL)
            *tt_rows = ((ttmode.ttdef_area.ttdef_bits.pagelength >= 5) ? (int)(ttmode.ttdef_area.ttdef_bits.pagelength)
                                                                       /* TT device value */
                                                                       : (24)); /* VT 100 default  */
        if (tt_cols != NULL)
            *tt_cols = ((ttmode.pagewidth >= 10) ? (int)(ttmode.pagewidth) /* TT device value */
                                                 : (80));                  /* VT 100 default  */
        if (tt_wrap != NULL)
...
void version(__G) __GDEF {
    ...
    for (chrp2 = &vms_vers[1]; chrp2 < chrp1; ver_maj = ver_maj * 10 + *(chrp2++) - '0')
        ;
#endif /* VMS_VERSION */
    ...
    len = sprintf((char*)slide, LoadFarString(CompiledWith),
                  ...
                  "DEC C",
#ifdef __DECC_VER
                  (sprintf(buf2, " %c%d.%d-%03d", ((vtyp = (__DECC_VER / 10000) % 10) == 6 ? 'T' : (vtyp == 8 ? 'S' : 'V')), __DECC_VER / 10000000, (__DECC_VER % 10000000) / 100000,
                           __DECC_VER % 1000),
                   buf2),
#else
                  "",
...
#ifdef VMS_VERSION
#if defined(__alpha)
                  "OpenVMS", (sprintf(buf, " (%s Alpha)", vms_vers), buf),
#elif defined(__ia64)
                  "OpenVMS", (sprintf(buf, " (%s IA64)", vms_vers), buf),
#else /* VAX */
                  (ver_maj >= 6) ? "OpenVMS" : "VMS", (sprintf(buf, " (%s VAX)", vms_vers), buf),
#endif
#else
                  "VMS", "",
#endif /* ?VMS_VERSION */
...
int spare[8] = {0};
globaldef{"LIB$INITIALIZE"} readonly _align(LONGWORD) void (*x_decc_init)() = decc_init;

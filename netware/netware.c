...
void InitUnZipConsole() {
    ...
    unsigned int* activeScreen = ImportSymbol(myHandle, "activeScreen");
    unsigned int* systemConsoleScreen = ImportSymbol(myHandle, "systemConsoleScreen");
    void (*pUseAccurateCaseForPaths)(int) = ImportSymbol(myHandle, "UseAccurateCaseForPaths");

    if (!activeScreen || !systemConsoleScreen || *activeScreen == *systemConsoleScreen)
...
#define FUNCTION (flag & FN_MASK)
    ...
    if (FUNCTION == ROOT) {
        ...
        if ((rootlen = strlen(pathcomp)) > 0) {
            ...
            if (rootlen > 0 && (stat(pathcomp, &G.statbuf) || !S_ISDIR(G.statbuf.st_mode))) /* path does not exist */
            {
...
void close_outfile(__G) /* GRR: change to return PK-style warning level */
    ...
    if (uO.D_flag <= 1) {
        ...
        if (ChangeDirectoryEntry(G.filename, &changeBuffer, MLastAccessedDateBit | MLastUpdatedDateBit | MLastUpdatedTimeBit, 0)) {
            if (uO.qflag)

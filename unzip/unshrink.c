...
static inline int HOT emit_bytes(__GPRO__ const uch* src, unsigned len, uch** outp, unsigned* cnt, uch* realbuf, unsigned outbufsiz) {
...
#define realbuf G.outbuf
    ...
    *G.outptr++ = finalval;
    ...
    while (1) {
        ...
        Trace((stderr, "code %4d; oldcode %4d; char %3d (%c); len %d; string [", curcode, oldcode, (int)(*newstr), (*newstr < 32 || *newstr >= 127) ? ' ' : *newstr, len));
        ...
        if (UNLIKELY(len <= 0)) {
            ...
            error = emit_bytes(__G__ newstr, (unsigned)len, &G.outptr, (unsigned*)&G.outcnt, realbuf, outbufsiz);
            if (UNLIKELY(error != 0)) {
...
static void COLD partial_clear(__G__ lastcodeused) __GDEF int lastcodeused;
{

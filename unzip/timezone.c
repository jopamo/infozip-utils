...
#define monthlen(m, y) (yr_days[0][(m) + 1] - yr_days[0][m] + ((m) == 1 && leap(y)))

/* internal module-level constants */
#ifndef IZ_MKTIME_ONLY
static ZCONST char gmt[] = "GMT";
static ZCONST int mon_lengths[2][MONSPERYEAR] = {{31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31}, {31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31}};
#endif /* !IZ_MKTIME_ONLY */
static ZCONST int yr_days[2][MONSPERYEAR + 1] = {{0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334, 365}, {0, 31, 60, 91, 121, 152, 182, 213, 244, 274, 305, 335, 366}};
#ifndef IZ_MKTIME_ONLY
static ZCONST int year_lengths[2] = {DAYSPERNYEAR, DAYSPERLYEAR};

/* internal variables */
static struct state statism;

/* prototypes of static functions */
static time_t transtime OF((ZCONST time_t janfirst, ZCONST int year, ZCONST struct rule* ZCONST rulep, ZCONST long offset));
static void generate_transitions OF((register struct state * ZCONST sp, ZCONST struct rule* ZCONST start, ZCONST struct rule* ZCONST end));
static ZCONST char* getzname OF((ZCONST char* strp));
static ZCONST char* getnum OF((ZCONST char* strp, int* ZCONST nump, ZCONST int min, ZCONST int max));
...
static void generate_transitions(sp, start, end) register struct state* ZCONST sp;
...
ZCONST char* strp;
{
    ...
    while ((c = *strp) != '\0' && !isdigit(c) && c != ',' && c != '-' && c != '+')
        ++strp;
    return strp;
}
...
void tzset() {
    ...
    if (old_TZstring && TZstring && !strcmp(old_TZstring, TZstring))
        ...
        return;
    if ((TZstring && TZstring[0] && Parse_TZ(TZstring, &statism) == 0) || IZTZ_GETLOCALETZINFO(&statism, generate_transitions) || Parse_TZ(gmt, &statism) == 0) {
...
ZCONST time_t* when;
{
    ...
    if (statism.timecnt == 0 || localwhen < statism.ats[0])
        timetype = statism.ttis[0].tt_isdst && statism.typecnt > 1 && !statism.ttis[1].tt_isdst;
    else {
...
struct tm* tb;
{
    ...
    do {
        ...
        if ((tzoffset_adj = localt - univt - statism.ttis[timetype].tt_gmtoff) == 0L)
            break;
        univt += tzoffset_adj;
    } while (--bailout_cnt > 0);
...
struct tm* tm;
{
    ...
    do {
        ltm = localtime(&then);
        if (ltm == (struct tm*)NULL || (tzoffset_adj = loctime - mkgmtime(ltm)) == 0L)
            break;
        then += tzoffset_adj;
    } while (--bailout_cnt > 0);
...
struct tm* tm;
{
    ...
    days = (unsigned)days + 365 * (unsigned)(years - EPOCH_YEAR) + (unsigned)(nleap(years));
    ...
    if (years > TM_YEAR_MAX || (years == TM_YEAR_MAX && (tm->tm_yday > (YDAYS(TM_MON_MAX, TM_YEAR_MAX) + (TM_MDAY_MAX - 1)) ||
                                                         (tm->tm_yday == (YDAYS(TM_MON_MAX, TM_YEAR_MAX) + (TM_MDAY_MAX - 1)) &&
                                                          (hours > TM_HOUR_MAX || (hours == TM_HOUR_MAX && (minutes > TM_MIN_MAX || (minutes == TM_MIN_MAX && seconds > TM_SEC_MAX))))))))
        return (time_t)-1;
#endif
#endif

    return (time_t)(SECSPERDAY * (unsigned long)(unsigned)days + SECSPERHOUR * (unsigned long)hours + (unsigned long)(SECSPERMIN * minutes + seconds));
}

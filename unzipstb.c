...
int main(int argc, char* argv[]) {
    ...
    printf("   UnZip versions:    expecting %u.%u%u, using %u.%u%u%s\n", UZ_MAJORVER, UZ_MINORVER, UZ_PATCHLEVEL, pVersion->unzip.major, pVersion->unzip.minor, pVersion->unzip.patchlevel,
           pVersion->betalevel);
    printf("   ZipInfo versions:  expecting %u.%u%u, using %u.%u%u\n", ZI_MAJORVER, ZI_MINORVER, UZ_PATCHLEVEL, pVersion->zipinfo.major, pVersion->zipinfo.minor, pVersion->zipinfo.patchlevel);
...
#define UZDLL_MINVERS_PATCHLEVEL 0
    ...
    if ((pVersion->unzip.major < UZDLL_MINVERS_MAJOR) ||
        ((pVersion->unzip.major == UZDLL_MINVERS_MAJOR) &&
         ((pVersion->unzip.minor < UZDLL_MINVERS_MINOR) || ((pVersion->unzip.minor == UZDLL_MINVERS_MINOR) && (pVersion->unzip.patchlevel < UZDLL_MINVERS_PATCHLEVEL))))) {
...
#endif /* ?WINDLL */
#endif /* ?OS2DLL */
        printf("   UnZip API version: can handle <= %u.%u%u, DLL supplies %u.%u%u\n", UZ_API_COMP_MAJOR, UZ_API_COMP_MINOR, UZ_API_COMP_REVIS, pVersion->dllapimin.major, pVersion->dllapimin.minor,
               pVersion->dllapimin.patchlevel);
        if ((pVersion->dllapimin.major > UZ_API_COMP_MAJOR) ||
            ((pVersion->dllapimin.major == UZ_API_COMP_MAJOR) &&
             ((pVersion->dllapimin.minor > UZ_API_COMP_MINOR) || ((pVersion->dllapimin.minor == UZ_API_COMP_MINOR) && (pVersion->dllapimin.patchlevel > UZ_API_COMP_REVIS))))) {

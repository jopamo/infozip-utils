...
char* do_wild(__G__ wildspec) __GDEF ZCONST char* wildspec; /* only used first time on a given dir */
{
    ...
    if (!notfirstcall) { /* first call:  must initialize everything */
        ...
        if ((wildname = strrchr(wildspec, '/')) == (ZCONST char*)NULL && (wildname = strrchr(wildspec, ':')) == (ZCONST char*)NULL) {
            dirname = ".";
            dirnamelen = 1;
            have_dirname = FALSE;
            wildname = wildspec;
            ...
            if ((dirname = (char*)malloc(dirnamelen + 1)) == (char*)NULL) {
                Info(slide, 1, ((char*)slide, LoadFarString(CantAllocateWildcard)));
                strncpy(matchname, wildspec, FILNAMSIZ);
                matchname[FILNAMSIZ - 1] = '\0';
...
static void map2fat(pathcomp, last_dot) char *pathcomp, *last_dot;
{
...
#define FUNCTION (flag & FN_MASK)
    ...
    if (FUNCTION == APPEND_NAME) {
        ...
        while ((*end = *pathcomp++) != '\0') {
            ...
            if ((end - buildpath) >= FILNAMSIZ) {
                *--end = '\0';
                Info(slide, 1, ((char*)slide, LoadFarString(PathTooLongTrunc), FnFilter1(G.filename), FnFilter2(buildpath)));
    ...
    if (FUNCTION == INIT) {
        ...
        if ((buildpath = (char*)malloc(strlen(G.filename) + rootlen + 3)) == (char*)NULL)
            return MPN_NOMEM;
 ...
 */
{
    ...
    if ((fnum = s_open(A_SET, G.filename)) < 0) {
        Info(slide, 0x201, ((char*)slide, "warning:  cannot open %s to set the time\n", FnFilter1(G.filename)));
        return;
    }
    ...
    if (s_get(T_FILE, fnum, &df, DSKFSIZE) < 0) {
        ...
        Info(slide, 0x201, ((char*)slide, "warning:  cannot get info on %s\n", FnFilter1(G.filename)));
        return;
    }
    ...
    if (s_set(T_FILE, fnum, &df, DSKFSIZE) < 0)
        Info(slide, 0x201, ((char*)slide, "warning:  cannot set info for %s\n", FnFilter1(G.filename)));
...
void version(__G) __GDEF {
    ...
    len = sprintf((char*)slide, LoadFarString(CompiledWith), "MetaWare High C", "", "FlexOS", " (16-bit, big)",

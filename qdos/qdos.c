...
int QReturn(int err) {
    ...
    if (err > 0)
        err = -err; /* We like -ve err nos (exclusively, alas) */
    exit(err);
}
...
#define FUNCTION (flag & FN_MASK)
    ...
    if (FUNCTION == ROOT) {
        ...
        if ((rootlen = strlen(pathcomp)) > 0) {
            ...
            if ((stat(tmproot, &G.statbuf) || !S_ISDIR(G.statbuf.st_mode))) { /* path does not exist */
...
static void qfix(__G__ ef_ptr, ef_len) __GDEF uch* ef_ptr;
unsigned ef_len;
{
    qdosextra qextra;
    ...
    while (ef_len >= EB_HEADSIZE) {
        ...
        if (eb_len > (ef_len - EB_HEADSIZE)) {
            ...
            Trace((stderr, "qfix: block length %u > rest ef_size %u\n", eb_len, ef_len - EB_HEADSIZE));
            break;
        }
            ...
            case SHORTID:
                if (!strncmp(extra->longid, LONGID, strlen(LONGID))) {
                    ...
                    if (extra->header.d_type) {
                        fs_heads(fgetchid(G.outfile), (timeout_t)-1, &extra->header, 14);
                ...
                if (!strncmp(jbp->longid, JBLONGID, strlen(JBLONGID))) {
                    ...
                    if (jbp->header.d_type) {
                        fs_heads(fgetchid(G.outfile), (timeout_t)-1, &jbp->header, 14);
                        G.pInfo->file_attr |= S_IXUSR;
                    }
                }
                break;
...
void version(__G) __GDEF {
    sprintf((char*)slide, LoadFarString(CompiledWith), "c68", " v4.2x", "SMS/QDOS", " on ", __DATE__, "", "");

...
static ZCONST char Far HeadersL[] = " Length   Method    Size  Cmpr    Date    Time   CRC-32   Name";
static ZCONST char Far HeadersL1[] = "--------  ------  ------- ---- ---------- ----- --------  ----";
static ZCONST char Far* Headers[][2] = {{HeadersS, HeadersS1}, {HeadersL, HeadersL1}};

static ZCONST char Far CaseConversion[] = "%s (\"^\" ==> case\n%s   conversion)\n";
static ZCONST char Far LongHdrStats[] = "%s  %-7s%s %4s %02u%c%02u%c%02u %02u:%02u %08lx %c";
...
int list_files(__G) /* return PK-type error code */
    ...
    static ZCONST char Far method[NUM_METHODS + 1][8] = {"Stored", "Shrunk",  "Reduce1", "Reduce2", "Reduce3", "Reduce4", "Implode", "Token", "Defl:#",
                                                         "Def64#", "ImplDCL", "BZip2",   "LZMA",    "Terse",   "IBMLZ77", "WavPack", "PPMd",  "Unk:###"};
    ...
    if (uO.qflag < 2) {
        if (uO.L_flag)
            Info(slide, 0, ((char*)slide, LoadFarString(CaseConversion), LoadFarStringSmall(Headers[longhdr][0]), LoadFarStringSmall2(Headers[longhdr][1])));
        else
            Info(slide, 0, ((char*)slide, "%s\n%s\n", LoadFarString(Headers[longhdr][0]), LoadFarStringSmall(Headers[longhdr][1])));
    }
        ...
        if (G.process_all_files || do_this_file) {
            ...
            if (methnum < NUM_METHODS) {
                ...
                if (G.crec.compression_method <= 999)
                    snprintf(methbuf, sizeof(methbuf), "Unk:%03u", G.crec.compression_method);
                else
                    snprintf(methbuf, sizeof(methbuf), "Unk:%04X", G.crec.compression_method);
            }
            ...
            if (longhdr)
                Info(slide, 0,
                     ((char*)slide, LoadFarString(LongHdrStats), FmZofft(G.crec.ucsize, "8", "u"), methbuf, FmZofft(csiz, "8", "u"), cfactorstr, disp_m, dt_sepchar, disp_d, dt_sepchar, disp_y, hh, mm,
                      G.crec.crc32, (G.pInfo->lcflag ? '^' : ' ')));
            else
                Info(slide, 0, ((char*)slide, LoadFarString(ShortHdrStats), FmZofft(G.crec.ucsize, "9", "u"), disp_m, dt_sepchar, disp_d, dt_sepchar, disp_y, hh, mm, (G.pInfo->lcflag ? '^' : ' ')));

            fnprint(__G);

            if ((error = do_string(__G__ G.crec.file_comment_length, QCOND ? DISPL_8 : SKIP)) != 0) {
                error_in_archive = error;
                if (error > PK_WARN)
                    return error;
            }
    ...
    if (uO.qflag < 2) {
        ...
        if (longhdr) {
            Info(slide, 0, ((char*)slide, LoadFarString(LongFileTrailer), FmZofft(tot_ucsize, "8", "u"), FmZofft(tot_csize, "8", "u"), cfactorstr, members, members == 1 ? "" : "s"));
        }
        else
            Info(slide, 0, ((char*)slide, LoadFarString(ShortFileTrailer), FmZofft(tot_ucsize, "9", "u"), members, members == 1 ? "" : "s"));
    }

    if (error_in_archive <= PK_WARN) {
        if ((memcmp(G.sig, (G.ecrec.have_ecr64 ? end_central64_sig : end_central_sig), 4) != 0) && (!G.ecrec.is_zip64_archive) && (memcmp(G.sig, end_central_sig, 4) != 0)) {
...
ulg* nmember;
{
    ...
    *nmember = 0L;
    ...
    for (j = 1L;; j++) {
        ...
        if (memcmp(G.sig, central_hdr_sig, 4)) {
            if (((unsigned)(j - 1) & (unsigned)0xFFFF) == (unsigned)G.ecrec.total_entries_central_dir) {
                break;
            }
            else {
                Info(slide, 0x401, ((char*)slide, LoadFarString(CentSigMsg), j));
...
int ratio(zusz_t uc, zusz_t c) {
    ...
    if (uc > 2000000L) {
        denom = uc / 1000L;
        return ((uc >= c) ? (int)((uc - c + (denom >> 1)) / denom) : -(int)((c - uc + (denom >> 1)) / denom));
    }
    else {
        denom = uc;
        return ((uc >= c) ? (int)((1000L * (uc - c) + (denom >> 1)) / denom) : -(int)((1000L * (c - uc) + (denom >> 1)) / denom));
    }
}

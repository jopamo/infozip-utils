...
error : Inconsistent flags,
        DYNALLOC_CRCTAB without DYNAMIC_CRC_TABLE.
#endif
...
#ifdef IZ_CRCOPTIM_UNFOLDTBL
#ifdef IZ_CRC_BE_OPTIMIZ
#define DO_OPT4(c, buf4) \
    c ^= *(buf4)++;      \
    c = crc_32_tab[c & 0xff] ^ crc_32_tab[256 + ((c >> 8) & 0xff)] ^ crc_32_tab[2 * 256 + ((c >> 16) & 0xff)] ^ crc_32_tab[3 * 256 + (c >> 24)]
#else /* !IZ_CRC_BE_OPTIMIZ */
#define DO_OPT4(c, buf4) \
    c ^= *(buf4)++;      \
    c = crc_32_tab[3 * 256 + (c & 0xff)] ^ crc_32_tab[2 * 256 + ((c >> 8) & 0xff)] ^ crc_32_tab[256 + ((c >> 16) & 0xff)] ^ crc_32_tab[c >> 24]
#endif /* ?IZ_CRC_BE_OPTIMIZ */
